...
;; mlkem__bytedecodedu__bytestobits__r__aggregate_def
(declare-fun mlkem__bytedecodedu__bytestobits__r__aggregate_def ((_ BitVec 8)) (Array Int u8_bit))

;; def_axiom
(assert
  (forall ((temp___2804 (_ BitVec 8)))
    (forall ((temp___2805 Int))
      (=>
        (in_range temp___2804)
        (= (to_rep1
             (select (mlkem__bytedecodedu__bytestobits__r__aggregate_def
                       temp___2804) temp___2805)) temp___2804)))))

;; Goal def'vc
;; File "mlkem.adb", line 1062, characters 0-0
(assert
  (not
  (forall ((this_bit (_ BitVec 8)))
    (forall ((r (Array Int u8_bit)))
      (=>
        (= r (mlkem__bytedecodedu__bytestobits__r__aggregate_def #x00))
        (=>
          (in_range this_bit)
          (forall ((i Int))
            (=>
              (= i 0)
              (=>
                (= (and (ite (<= 0 i) true false) (ite (<= i 319) true false)) true)
