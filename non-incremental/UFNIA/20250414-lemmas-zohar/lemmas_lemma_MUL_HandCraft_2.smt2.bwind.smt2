...
Application: Proving correctness of bit-vector lemmas
Target solver: cvc5
Publications: Niemetz, A., Preiner, M. and Zohar, Y., 2024, July. Scalable bit-blasting with abstractions. In International Conference on Computer Aided Verification (pp. 178-200). Cham: Springer Nature Switzerland.
Time limit: 60
Benchmarks that represent integer encodings of bit-vector lemmas.
|)
...
(define-fun and_ind_def () Bool (forall ((k Int) (a Int) (b Int)) (=> (and (> k 0) (in_range k a) (in_range k b)) (= (intand k a b) (+ (ite (> k 1) (intand (- k 1) (int_all_but_msb k a) (int_all_but_msb k b)) 0) (* (pow2 (- k 1)) (intand_helper (bitof k (- k 1) a) (bitof k (- k 1) b)))))) ) )
(define-fun or_ind_def ()  Bool (forall ((k Int) (a Int) (b Int))  (=> (and (> k 0) (in_range k a) (in_range k b)) (= (intor k a b) (+ (ite (> k 1) (intor (- k 1) (int_all_but_msb k a) (int_all_but_msb k b)) 0) (* (pow2 (- k 1)) (intor_helper (bitof k (- k 1) a) (bitof k (- k 1) b)))))) ) )
(define-fun xor_ind_def () Bool (forall ((k Int) (a Int) (b Int))  (=> (and (> k 0) (in_range k a) (in_range k b)) (= (intxor k a b) (+ (ite (> k 1) (intxor (- k 1) (int_all_but_msb k a) (int_all_but_msb k b)) 0) (* (pow2 (- k 1)) (intxor_helper (bitof k (- k 1) a) (bitof k (- k 1) b))))))  ))
;pow2 properties
...
(define-fun and_ax ((k Int)) Bool (and and_ind_def (and_properties k)))
(define-fun or_ax ((k Int)) Bool (and or_ind_def (or_properties k)))
(define-fun xor_ax ((k Int)) Bool (and xor_ind_def (xor_properties k)))
...
(declare-const t Int)
(declare-const i Int)
(define-fun lemma_MUL_HC2 ((x Int) (s Int) (t Int)) Bool (=> (= s (intneg k (intshl k 1 i))) (= t (intshl k (intneg k x) i))))
(assert (>= i 0))
(assert (< i k))
(assert (= t (intmul k x s)))
(assert (not (lemma_MUL_HC2 x s t)))
; bounds

(assert (and (<= 0 x) (< x (pow2 k))))
(assert (and (<= 0 s) (< s (pow2 k))))
(assert (and (<= 0 t) (< t (pow2 k))))
(assert (and (<= 0 i) (< i (pow2 k))))
