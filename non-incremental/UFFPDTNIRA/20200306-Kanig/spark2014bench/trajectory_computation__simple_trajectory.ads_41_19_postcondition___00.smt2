...
;; defqtvc
 ;; File "simple_trajectory.ads", line 33, characters 0-0
  ...
  bound) new_speed1)
  (=>
  (fp.leq (fp.mul RNE ((_ to_fp 11 53) RNE (to_real (+ n 1))) (fp.neg (fp #b0 #b10000000101 #b0000010000000000000000000000000000000000000000000000))) new_speed1)
  (=>
  (fp.leq new_speed1 (fp.add RNE (fp.mul RNE ((_ to_fp 11 53) RNE (to_real 
  n)) bound) bound))
  (=>
  (fp.leq new_speed1 (fp.mul RNE ((_ to_fp 11 53) RNE (to_real (+ n 1))) 
  bound)) (= (invariant__ (+ n 1) new_speed1) true)))))))))))))))))))))))))))))))))
(check-sat)
(exit)
