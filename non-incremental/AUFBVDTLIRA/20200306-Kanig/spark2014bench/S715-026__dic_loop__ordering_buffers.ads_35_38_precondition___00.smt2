...
;; has_message__post_axiom
  (assert
  (and
  (forall ((buffer1 us_rep)) (first__function_guard (first buffer1) buffer1))
  (forall ((buffer1 us_rep)) (last__function_guard (last buffer1) buffer1))))
...
;; has_message__def_axiom
  ...
  (forall ((index1 Int))
  (! (and
     (forall ((buffer2 us_rep) (index2 Int)) (get_message__function_guard
     (get_message buffer2 index2) buffer2 index2))
     (and
     (forall ((buffer2 us_rep) (index2 Int)) (get_message__function_guard
     (get_message buffer2 index2) buffer2 index2))
     (and
     (forall ((buffer2 us_rep) (index2 Int)) (is_null__function_guard
     (is_null (get_message buffer2 index2)) (get_message buffer2 index2)))
     (= (= (has_message buffer1 index1) true)
     (not (= (is_null (get_message buffer1 index1)) true)))))) :pattern (
  (has_message buffer1 index1)) ))))
...
(assert
;; defqtvc
 ;; File "ordering_buffers.ads", line 32, characters 0-0
  (not
  (=> (dynamic_invariant2 index true false true true)
  (=> (first__function_guard (first buffer) buffer)
  (=> (<= (first buffer) index)
  (=> (last__function_guard (last buffer) buffer)
  (=> (<= index (last buffer))
