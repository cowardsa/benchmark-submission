...
;; readmem8__post_axiom
  ...
  (! (=> (dynamic_invariant addr1 true true true true)
     ...
     (and (= result (to_rep (select x86__memory addr1))) (dynamic_invariant2
     result true false true true))))) :pattern ((readmem8 addr1 x86__memory)) ))))

(assert
;; defqtvc
 ;; File "x86.ads", line 430, characters 0-0
  (not
  (forall ((memory (Array (_ BitVec 64) unsigned8)) (tmp (_ BitVec 16)))
  ...
  (readmem16__function_guard (readmem16 addr memory5) addr memory5))
  (= (readmem16 addr memory4) val__)))))))))))))))))))))))
(check-sat)
(exit)
