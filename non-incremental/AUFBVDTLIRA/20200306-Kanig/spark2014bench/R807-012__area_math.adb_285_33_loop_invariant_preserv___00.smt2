(set-info :smt-lib-version 2.7)
(set-logic AUFBVDTLIRA)
(set-info :source |
Generated by: AdaCore
Generated on: 2020-03-06
Generator: SPARK
Application: Program verification for Ada
Target solver: CVC4
|)
(set-info :license "https://creativecommons.org/licenses/by/4.0/")
(set-info :category "industrial")
(set-info :status unknown)
;;; generated by SMT-LIB2 driver
;;; SMT-LIB2 driver: bit-vectors, common part
;;; SMT-LIB2: integer arithmetic
(declare-sort string 0)

(declare-datatypes ((tuple0 0))
(((Tuple0))))
(declare-sort us_private 0)

(declare-fun private__bool_eq (us_private us_private) Bool)

(declare-const us_null_ext__ us_private)

(declare-sort us_type_of_heap 0)

(declare-datatypes ((us_type_of_heap__ref 0))
(((us_type_of_heap__refqtmk (us_type_of_heap__content us_type_of_heap)))))
(declare-sort us_image 0)

(declare-datatypes ((int__ref 0))
(((int__refqtmk (int__content Int)))))
(declare-datatypes ((bool__ref 0))
(((bool__refqtmk (bool__content Bool)))))
(declare-datatypes ((us_fixed__ref 0))
(((us_fixed__refqtmk (us_fixed__content Int)))))
(declare-datatypes ((real__ref 0))
(((real__refqtmk (real__content Real)))))
(declare-datatypes ((us_private__ref 0))
(((us_private__refqtmk (us_private__content us_private)))))
(define-fun int__ref___projection ((a int__ref)) Int (int__content a))

(define-fun us_fixed__ref___projection ((a us_fixed__ref)) Int (us_fixed__content
                                                               a))

(define-fun bool__ref___projection ((a bool__ref)) Bool (bool__content a))

(define-fun real__ref___projection ((a real__ref)) Real (real__content a))

(define-fun us_private__ref___projection ((a us_private__ref)) us_private 
  (us_private__content a))

(declare-fun nth ((_ BitVec 32) Int) Bool)

(declare-fun lsr ((_ BitVec 32) Int) (_ BitVec 32))

(declare-fun asr ((_ BitVec 32) Int) (_ BitVec 32))

(declare-fun lsl ((_ BitVec 32) Int) (_ BitVec 32))

(define-fun abs1 ((x Int)) Int (ite (<= 0 x) x (- x)))

;; Abs_le
  (assert
  (forall ((x Int) (y Int)) (= (<= (abs1 x) y) (and (<= (- y) x) (<= x y)))))

;; Abs_pos
  (assert (forall ((x Int)) (<= 0 (abs1 x))))

;; Div_unique
  (assert
  (forall ((x Int) (y Int) (q Int))
  (=> (< 0 y) (=> (and (<= (* q y) x) (< x (+ (* q y) y))) (= (div x y) q)))))

;; Div_bound
  (assert
  (forall ((x Int) (y Int))
  (=> (and (<= 0 x) (< 0 y)) (and (<= 0 (div x y)) (<= (div x y) x)))))

;; Div_inf
  (assert
  (forall ((x Int) (y Int)) (=> (and (<= 0 x) (< x y)) (= (div x y) 0))))

;; Div_inf_neg
  (assert
  (forall ((x Int) (y Int))
  (=> (and (< 0 x) (<= x y)) (= (div (- x) y) (- 1)))))

;; Mod_0
  (assert (forall ((y Int)) (=> (not (= y 0)) (= (mod 0 y) 0))))

;; Div_1_left
  (assert (forall ((y Int)) (=> (< 1 y) (= (div 1 y) 0))))

;; Div_minus1_left
  (assert (forall ((y Int)) (=> (< 1 y) (= (div (- 1) y) (- 1)))))

;; Mod_1_left
  (assert (forall ((y Int)) (=> (< 1 y) (= (mod 1 y) 1))))

;; Mod_minus1_left
  (assert
  (forall ((y Int))
  (! (=> (< 1 y) (= (mod (- 1) y) (- y 1))) :pattern ((mod (- 1) y)) )))

;; Div_mult
  (assert
  (forall ((x Int) (y Int) (z Int))
  (! (=> (< 0 x) (= (div (+ (* x y) z) x) (+ y (div z x)))) :pattern ((div (+ (* x y) z) x)) )))

;; Mod_mult
  (assert
  (forall ((x Int) (y Int) (z Int))
  (! (=> (< 0 x) (= (mod (+ (* x y) z) x) (mod z x))) :pattern ((mod (+ (* x y) z) x)) )))

(declare-fun rotate_right1 ((_ BitVec 32) Int) (_ BitVec 32))

(declare-fun rotate_left1 ((_ BitVec 32) Int) (_ BitVec 32))

(declare-fun pow2 (Int) Int)

(define-fun to_int1 ((x (_ BitVec 32))) Int (ite (bvsge x (_ bv0 32))
                                            (ubv_to_int x)
                                            (- (- 4294967296 (ubv_to_int x)))))

(define-fun uint_in_range ((i Int)) Bool (and (<= 0 i) (<= i 4294967295)))

;; lsr_bv_is_lsr
  (assert
  (forall ((x (_ BitVec 32)) (n (_ BitVec 32)))
  (= (bvlshr x n) (lsr x (ubv_to_int n)))))

;; asr_bv_is_asr
  (assert
  (forall ((x (_ BitVec 32)) (n (_ BitVec 32)))
  (= (bvashr x n) (asr x (ubv_to_int n)))))

;; lsl_bv_is_lsl
  (assert
  (forall ((x (_ BitVec 32)) (n (_ BitVec 32)))
  (= (bvshl x n) (lsl x (ubv_to_int n)))))

;; rotate_left_bv_is_rotate_left
  (assert
  (forall ((v (_ BitVec 32)) (n (_ BitVec 32)))
  (= (bvor (bvshl v (bvurem n (_ bv32 32))) (bvlshr v (bvsub (_ bv32 32) (bvurem n (_ bv32 32))))) 
  (rotate_left1 v (ubv_to_int n)))))

;; rotate_right_bv_is_rotate_right
  (assert
  (forall ((v (_ BitVec 32)) (n (_ BitVec 32)))
  (= (bvor (bvlshr v (bvurem n (_ bv32 32))) (bvshl v (bvsub (_ bv32 32) (bvurem n (_ bv32 32))))) 
  (rotate_right1 v (ubv_to_int n)))))

(declare-fun nth_bv ((_ BitVec 32) (_ BitVec 32)) Bool)

;; nth_bv_def
  (assert
  (forall ((x (_ BitVec 32)) (i (_ BitVec 32)))
  (= (= (nth_bv x i) true)
  (not (= (bvand (bvlshr x i) #x00000001) #x00000000)))))

;; Nth_bv_is_nth
  (assert
  (forall ((x (_ BitVec 32)) (i (_ BitVec 32)))
  (= (nth x (ubv_to_int i)) (nth_bv x i))))

;; Nth_bv_is_nth2
  (assert
  (forall ((x (_ BitVec 32)) (i Int))
  (=> (and (<= 0 i) (< i 4294967296))
  (= (nth_bv x ((_ int_to_bv 32) i)) (nth x i)))))

(declare-fun eq_sub_bv ((_ BitVec 32) (_ BitVec 32) (_ BitVec 32)
  (_ BitVec 32)) Bool)

;; eq_sub_bv_def
  (assert
  (forall ((a (_ BitVec 32)) (b (_ BitVec 32)) (i (_ BitVec 32))
  (n (_ BitVec 32)))
  (let ((mask (bvshl (bvsub (bvshl #x00000001 n) #x00000001) i)))
  (= (eq_sub_bv a b i n) (= (bvand b mask) (bvand a mask))))))

(define-fun eq_sub ((a (_ BitVec 32)) (b (_ BitVec 32)) (i Int)
  (n Int)) Bool (forall ((j Int))
                (=> (and (<= i j) (< j (+ i n))) (= (nth a j) (nth b j)))))

;; eq_sub_equiv
  (assert
  (forall ((a (_ BitVec 32)) (b (_ BitVec 32)) (i (_ BitVec 32))
  (n (_ BitVec 32)))
  (= (eq_sub a b (ubv_to_int i) (ubv_to_int n)) (eq_sub_bv a b i n))))

(declare-datatypes ((t__ref 0))
(((t__refqtmk (t__content (_ BitVec 32))))))
(declare-fun power ((_ BitVec 32) Int) (_ BitVec 32))

(define-fun bv_min ((x (_ BitVec 32))
  (y (_ BitVec 32))) (_ BitVec 32) (ite (bvule x y) x y))

(define-fun bv_max ((x (_ BitVec 32))
  (y (_ BitVec 32))) (_ BitVec 32) (ite (bvule x y) y x))

(define-fun min ((x Int) (y Int)) Int (ite (<= x y) x y))

(define-fun max ((x Int) (y Int)) Int (ite (<= x y) y x))

;; Min_r
  (assert (forall ((x Int) (y Int)) (=> (<= y x) (= (min x y) y))))

;; Max_l
  (assert (forall ((x Int) (y Int)) (=> (<= y x) (= (max x y) x))))

;; Min_comm
  (assert (forall ((x Int) (y Int)) (= (min x y) (min y x))))

;; Max_comm
  (assert (forall ((x Int) (y Int)) (= (max x y) (max y x))))

;; Min_assoc
  (assert
  (forall ((x Int) (y Int) (z Int)) (= (min (min x y) z) (min x (min y z)))))

;; Max_assoc
  (assert
  (forall ((x Int) (y Int) (z Int)) (= (max (max x y) z) (max x (max y z)))))

(define-fun in_range ((x Int)) Bool (or (= x 0) (= x 1)))

(declare-fun attr__ATTRIBUTE_IMAGE (Bool) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE (us_image) Bool)

(declare-sort integer 0)

(declare-fun integerqtint (integer) Int)

;; integer'axiom
  (assert
  (forall ((i integer))
  (and (<= (- 2147483648) (integerqtint i)) (<= (integerqtint i) 2147483647))))

(define-fun in_range1 ((x Int)) Bool (and (<= (- 2147483648) x)
                                     (<= x 2147483647)))

(declare-fun attr__ATTRIBUTE_IMAGE1 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check1 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE1 (us_image) Int)

(declare-fun user_eq (integer integer) Bool)

(declare-const dummy integer)

(declare-datatypes ((integer__ref 0))
(((integer__refqtmk (integer__content integer)))))
(define-fun integer__ref_integer__content__projection ((a integer__ref)) integer 
  (integer__content a))

(declare-sort natural 0)

(declare-fun naturalqtint (natural) Int)

;; natural'axiom
  (assert
  (forall ((i natural))
  (and (<= 0 (naturalqtint i)) (<= (naturalqtint i) 2147483647))))

(define-fun in_range2 ((x Int)) Bool (and (<= 0 x) (<= x 2147483647)))

(declare-fun attr__ATTRIBUTE_IMAGE2 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check2 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE2 (us_image) Int)

(declare-fun user_eq1 (natural natural) Bool)

(declare-const dummy1 natural)

(declare-datatypes ((natural__ref 0))
(((natural__refqtmk (natural__content natural)))))
(define-fun natural__ref_natural__content__projection ((a natural__ref)) natural 
  (natural__content a))

(define-fun dynamic_invariant ((temp___expr_39 Int) (temp___is_init_35 Bool)
  (temp___skip_constant_36 Bool) (temp___do_toplevel_37 Bool)
  (temp___do_typ_inv_38 Bool)) Bool (=>
                                    (or (= temp___is_init_35 true)
                                    (<= 0 2147483647)) (in_range2
                                    temp___expr_39)))

(define-fun to_rep ((x natural)) Int (naturalqtint x))

(declare-fun of_rep (Int) natural)

;; inversion_axiom
  (assert
  (forall ((x natural)) (! (= (of_rep (to_rep x)) x) :pattern ((to_rep x)) )))

;; range_axiom
  (assert
  (forall ((x natural)) (! (in_range2 (to_rep x)) :pattern ((to_rep x)) )))

;; coerce_axiom
  (assert
  (forall ((x Int))
  (! (=> (in_range2 x) (= (to_rep (of_rep x)) x)) :pattern ((to_rep
                                                            (of_rep x))) )))

(define-fun to_rep1 ((x integer)) Int (integerqtint x))

(declare-fun of_rep1 (Int) integer)

;; inversion_axiom
  (assert
  (forall ((x integer))
  (! (= (of_rep1 (to_rep1 x)) x) :pattern ((to_rep1 x)) )))

;; range_axiom
  (assert
  (forall ((x integer)) (! (in_range1 (to_rep1 x)) :pattern ((to_rep1 x)) )))

;; coerce_axiom
  (assert
  (forall ((x Int))
  (! (=> (in_range1 x) (= (to_rep1 (of_rep1 x)) x)) :pattern ((to_rep1
                                                              (of_rep1 x))) )))

(declare-sort address_type 0)

(declare-const attr__ATTRIBUTE_MODULUS (_ BitVec 32))

(declare-fun attr__ATTRIBUTE_IMAGE3 ((_ BitVec 32)) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check3 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE3 (us_image) (_ BitVec 32))

(declare-fun user_eq2 (address_type address_type) Bool)

(declare-const dummy2 address_type)

(declare-datatypes ((address_type__ref 0))
(((address_type__refqtmk (address_type__content address_type)))))
(define-fun address_type__ref_address_type__content__projection ((a address_type__ref)) address_type 
  (address_type__content a))

(declare-fun to_rep2 (address_type) (_ BitVec 32))

(declare-fun of_rep2 ((_ BitVec 32)) address_type)

;; inversion_axiom
  (assert
  (forall ((x address_type))
  (! (= (of_rep2 (to_rep2 x)) x) :pattern ((to_rep2 x)) )))

;; range_axiom
  (assert true)

;; coerce_axiom
  (assert
  (forall ((x (_ BitVec 32)))
  (! (= (to_rep2 (of_rep2 x)) x) :pattern ((to_rep2 (of_rep2 x))) )))

(define-fun to_int2 ((x address_type)) Int (ubv_to_int (to_rep2 x)))

;; range_int_axiom
  (assert
  (forall ((x address_type)) (! (uint_in_range
  (to_int2 x)) :pattern ((to_int2 x)) )))

(declare-datatypes ((map__ref 0))
(((map__refqtmk (map__content (Array Int address_type))))))
(declare-fun slide ((Array Int address_type) Int
  Int) (Array Int address_type))

;; slide_eq
  (assert
  (forall ((a (Array Int address_type)))
  (forall ((first Int))
  (! (= (slide a first first) a) :pattern ((slide a first first)) ))))

;; slide_def
  (assert
  (forall ((a (Array Int address_type)))
  (forall ((old_first Int))
  (forall ((new_first Int))
  (forall ((i Int))
  (! (= (select (slide a old_first new_first) i) (select a (- i (- new_first old_first)))) :pattern ((select 
  (slide a old_first new_first) i)) ))))))

(define-fun bool_eq ((a (Array Int address_type)) (a__first Int)
  (a__last Int) (b (Array Int address_type)) (b__first Int)
  (b__last Int)) Bool (ite (and
                           (ite (<= a__first a__last)
                           (and (<= b__first b__last)
                           (= (- a__last a__first) (- b__last b__first)))
                           (< b__last b__first))
                           (forall ((temp___idx_154 Int))
                           (=>
                           (and (<= a__first temp___idx_154)
                           (<= temp___idx_154 a__last))
                           (= (to_rep2 (select a temp___idx_154)) (to_rep2
                                                                  (select b (+ (- b__first a__first) temp___idx_154)))))))
                      true false))

;; bool_eq_rev
  (assert
  (forall ((a (Array Int address_type)) (b (Array Int address_type)))
  (forall ((a__first Int) (a__last Int) (b__first Int) (b__last Int))
  (=> (= (bool_eq b b__first b__last a a__first a__last) true)
  (and
  (ite (<= a__first a__last)
  (and (<= b__first b__last) (= (- a__last a__first) (- b__last b__first)))
  (< b__last b__first))
  (forall ((temp___idx_154 Int))
  (=> (and (<= a__first temp___idx_154) (<= temp___idx_154 a__last))
  (= (to_rep2 (select a temp___idx_154)) (to_rep2
                                         (select b (+ (- b__first a__first) temp___idx_154)))))))))))

(declare-sort t 0)

(declare-fun first (t) integer)

(declare-fun last (t) integer)

(declare-fun mk (Int Int) t)

;; mk_def
  (assert
  (forall ((f Int) (l Int))
  (! (=> (in_range1 f)
     (=> (in_range1 l)
     (and (= (to_rep1 (first (mk f l))) f) (= (to_rep1 (last (mk f l))) l)))) :pattern (
  (mk f l)) )))

(define-fun dynamic_property ((range_first Int) (range_last Int) (low Int)
  (high Int)) Bool (and (in_range1 low)
                   (and (in_range1 high)
                   (=> (<= low high) (and (in_range1 low) (in_range1 high))))))

(declare-datatypes ((us_t 0))
(((us_tqtmk (elts (Array Int address_type))(rt t)))))
(define-fun to_array ((a us_t)) (Array Int address_type) (elts a))

(define-fun of_array ((a (Array Int address_type)) (f Int)
  (l Int)) us_t (us_tqtmk a (mk f l)))

(define-fun first1 ((a us_t)) Int (to_rep1 (first (rt a))))

(define-fun last1 ((a us_t)) Int (to_rep1 (last (rt a))))

(define-fun length ((a us_t)) Int (ite (<= (first1 a) (last1 a))
                                  (+ (- (last1 a) (first1 a)) 1) 0))

(declare-const value__size Int)

(declare-const object__size Int)

(declare-const component__size Int)

(declare-const alignment Int)

;; value__size_axiom
  (assert (<= 0 value__size))

;; object__size_axiom
  (assert (<= 0 object__size))

;; component__size_axiom
  (assert (<= 0 component__size))

;; alignment_axiom
  (assert (<= 0 alignment))

(define-fun bool_eq1 ((x us_t)
  (y us_t)) Bool (bool_eq (elts x) (to_rep1 (first (rt x)))
                 (to_rep1 (last (rt x))) (elts y) (to_rep1 (first (rt y)))
                 (to_rep1 (last (rt y)))))

(declare-fun user_eq3 (us_t us_t) Bool)

(declare-const dummy3 us_t)

(declare-datatypes ((area_array__ref 0))
(((area_array__refqtmk (area_array__content us_t)))))
(define-fun area_array__ref_area_array__content__projection ((a area_array__ref)) us_t 
  (area_array__content a))

(declare-datatypes ((t6s__ref 0))
(((t6s__refqtmk (t6s__content us_t)))))
(define-fun t6s__ref_t6s__content__projection ((a t6s__ref)) us_t (t6s__content
                                                                  a))

(declare-datatypes ((t8s__ref 0))
(((t8s__refqtmk (t8s__content us_t)))))
(define-fun t8s__ref_t8s__content__projection ((a t8s__ref)) us_t (t8s__content
                                                                  a))

(declare-datatypes ((us_split_discrs 0))
(((us_split_discrsqtmk (rec__area_math__ensemble__max natural)))))
(define-fun us_split_discrs_rec__area_math__ensemble__max__projection ((a us_split_discrs)) natural 
  (rec__area_math__ensemble__max a))

(declare-datatypes ((us_split_discrs__ref 0))
(((us_split_discrs__refqtmk (us_split_discrs__content us_split_discrs)))))
(define-fun us_split_discrs__ref___split_discrs__content__projection ((a us_split_discrs__ref)) us_split_discrs 
  (us_split_discrs__content a))

(declare-datatypes ((us_split_fields 0))
(((us_split_fieldsqtmk
  (rec__area_math__ensemble__size natural)(rec__area_math__ensemble__from us_t)(rec__area_math__ensemble__to us_t)))))
(define-fun us_split_fields_rec__area_math__ensemble__size__projection ((a us_split_fields)) natural 
  (rec__area_math__ensemble__size a))

(define-fun us_split_fields_rec__area_math__ensemble__from__projection ((a us_split_fields)) us_t 
  (rec__area_math__ensemble__from a))

(define-fun us_split_fields_rec__area_math__ensemble__to__projection ((a us_split_fields)) us_t 
  (rec__area_math__ensemble__to a))

(declare-datatypes ((us_split_fields__ref 0))
(((us_split_fields__refqtmk (us_split_fields__content us_split_fields)))))
(define-fun us_split_fields__ref___split_fields__content__projection ((a us_split_fields__ref)) us_split_fields 
  (us_split_fields__content a))

(declare-datatypes ((us_rep 0))
(((us_repqtmk
  (us_split_discrs1 us_split_discrs)(us_split_fields1 us_split_fields)))))
(define-fun us_rep___split_discrs__projection ((a us_rep)) us_split_discrs 
  (us_split_discrs1 a))

(define-fun us_rep___split_fields__projection ((a us_rep)) us_split_fields 
  (us_split_fields1 a))

(define-fun bool_eq2 ((a us_rep)
  (b us_rep)) Bool (ite (and
                        (= (to_rep
                           (rec__area_math__ensemble__max
                           (us_split_discrs1 a))) (to_rep
                                                  (rec__area_math__ensemble__max
                                                  (us_split_discrs1 b))))
                        (and
                        (and
                        (= (to_rep
                           (rec__area_math__ensemble__size
                           (us_split_fields1 a))) (to_rep
                                                  (rec__area_math__ensemble__size
                                                  (us_split_fields1 b))))
                        (= (bool_eq1
                           (rec__area_math__ensemble__from
                           (us_split_fields1 a))
                           (rec__area_math__ensemble__from
                           (us_split_fields1 b))) true))
                        (= (bool_eq1
                           (rec__area_math__ensemble__to
                           (us_split_fields1 a))
                           (rec__area_math__ensemble__to
                           (us_split_fields1 b))) true)))
                   true false))

(define-fun in_range3 ((rec__area_math__ensemble__max1 Int)
  (a us_split_discrs)) Bool (= rec__area_math__ensemble__max1 (to_rep
                                                              (rec__area_math__ensemble__max
                                                              a))))

(declare-const value__size1 Int)

(declare-const object__size1 Int)

(declare-const alignment1 Int)

;; value__size_axiom
  (assert (<= 0 value__size1))

;; object__size_axiom
  (assert (<= 0 object__size1))

;; alignment_axiom
  (assert (<= 0 alignment1))

(declare-const area_math__ensemble__max__first__bit Int)

(declare-const area_math__ensemble__max__last__bit Int)

(declare-const area_math__ensemble__max__position Int)

;; area_math__ensemble__max__first__bit_axiom
  (assert (<= 0 area_math__ensemble__max__first__bit))

;; area_math__ensemble__max__last__bit_axiom
  (assert
  (< area_math__ensemble__max__first__bit area_math__ensemble__max__last__bit))

;; area_math__ensemble__max__position_axiom
  (assert (<= 0 area_math__ensemble__max__position))

(declare-const area_math__ensemble__size__first__bit Int)

(declare-const area_math__ensemble__size__last__bit Int)

(declare-const area_math__ensemble__size__position Int)

;; area_math__ensemble__size__first__bit_axiom
  (assert (<= 0 area_math__ensemble__size__first__bit))

;; area_math__ensemble__size__last__bit_axiom
  (assert
  (< area_math__ensemble__size__first__bit area_math__ensemble__size__last__bit))

;; area_math__ensemble__size__position_axiom
  (assert (<= 0 area_math__ensemble__size__position))

(declare-const area_math__ensemble__from__first__bit Int)

(declare-const area_math__ensemble__from__last__bit Int)

(declare-const area_math__ensemble__from__position Int)

;; area_math__ensemble__from__first__bit_axiom
  (assert (<= 0 area_math__ensemble__from__first__bit))

;; area_math__ensemble__from__last__bit_axiom
  (assert
  (< area_math__ensemble__from__first__bit area_math__ensemble__from__last__bit))

;; area_math__ensemble__from__position_axiom
  (assert (<= 0 area_math__ensemble__from__position))

(declare-const area_math__ensemble__to__first__bit Int)

(declare-const area_math__ensemble__to__last__bit Int)

(declare-const area_math__ensemble__to__position Int)

;; area_math__ensemble__to__first__bit_axiom
  (assert (<= 0 area_math__ensemble__to__first__bit))

;; area_math__ensemble__to__last__bit_axiom
  (assert
  (< area_math__ensemble__to__first__bit area_math__ensemble__to__last__bit))

;; area_math__ensemble__to__position_axiom
  (assert (<= 0 area_math__ensemble__to__position))

(declare-fun user_eq4 (us_rep us_rep) Bool)

(declare-const dummy4 us_rep)

(declare-datatypes ((ensemble__ref 0))
(((ensemble__refqtmk (ensemble__content us_rep)))))
(define-fun ensemble__ref_ensemble__content__projection ((a ensemble__ref)) us_rep 
  (ensemble__content a))

(declare-fun is_consistent (us_rep) Bool)

(declare-fun is_consistent__function_guard (Bool us_rep) Bool)

(define-fun dynamic_invariant1 ((temp___expr_206 us_rep)
  (temp___is_init_202 Bool) (temp___skip_constant_203 Bool)
  (temp___do_toplevel_204 Bool)
  (temp___do_typ_inv_205 Bool)) Bool (and
                                     (=> (= temp___do_toplevel_204 true)
                                     (=> (= temp___is_init_202 true)
                                     (and
                                     (<= (to_rep
                                         (rec__area_math__ensemble__size
                                         (us_split_fields1 temp___expr_206))) 
                                     (last1
                                     (rec__area_math__ensemble__from
                                     (us_split_fields1 temp___expr_206))))
                                     (<= (to_rep
                                         (rec__area_math__ensemble__size
                                         (us_split_fields1 temp___expr_206))) 
                                     (last1
                                     (rec__area_math__ensemble__to
                                     (us_split_fields1 temp___expr_206)))))))
                                     (let ((temp___208 (rec__area_math__ensemble__max
                                                       (us_split_discrs1
                                                       temp___expr_206))))
                                     (and
                                     (and (dynamic_property 1
                                     (to_rep temp___208)
                                     (first1
                                     (rec__area_math__ensemble__from
                                     (us_split_fields1 temp___expr_206)))
                                     (last1
                                     (rec__area_math__ensemble__from
                                     (us_split_fields1 temp___expr_206))))
                                     (and
                                     (= (first1
                                        (rec__area_math__ensemble__from
                                        (us_split_fields1 temp___expr_206))) 1)
                                     (= (last1
                                        (rec__area_math__ensemble__from
                                        (us_split_fields1 temp___expr_206))) 
                                     (to_rep temp___208))))
                                     (and (dynamic_property 1
                                     (to_rep temp___208)
                                     (first1
                                     (rec__area_math__ensemble__to
                                     (us_split_fields1 temp___expr_206)))
                                     (last1
                                     (rec__area_math__ensemble__to
                                     (us_split_fields1 temp___expr_206))))
                                     (and
                                     (= (first1
                                        (rec__area_math__ensemble__to
                                        (us_split_fields1 temp___expr_206))) 1)
                                     (= (last1
                                        (rec__area_math__ensemble__to
                                        (us_split_fields1 temp___expr_206))) 
                                     (to_rep temp___208))))))))

(define-fun dynamic_predicate ((temp___211 us_rep)) Bool (and
                                                         (<= (to_rep
                                                             (rec__area_math__ensemble__size
                                                             (us_split_fields1
                                                             temp___211))) 
                                                         (last1
                                                         (rec__area_math__ensemble__from
                                                         (us_split_fields1
                                                         temp___211))))
                                                         (<= (to_rep
                                                             (rec__area_math__ensemble__size
                                                             (us_split_fields1
                                                             temp___211))) 
                                                         (last1
                                                         (rec__area_math__ensemble__to
                                                         (us_split_fields1
                                                         temp___211))))))

;; is_consistent__post_axiom
  (assert true)

;; is_consistent__def_axiom
  (assert
  (forall ((e us_rep))
  (! (= (= (is_consistent e) true)
     (and
     (forall ((r Int))
     (=>
     (and (<= 1 r)
     (<= r (- (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) 1)))
     (and
     (bvult (to_rep2
            (let ((temp___225 (rec__area_math__ensemble__to
                              (us_split_fields1 e))))
            (select (to_array temp___225) r))) (to_rep2
                                               (let ((temp___226 (rec__area_math__ensemble__from
                                                                 (us_split_fields1
                                                                 e))))
                                               (select (to_array temp___226) (+ r 1)))))
     (bvugt (bvsub (to_rep2
                   (let ((temp___227 (rec__area_math__ensemble__from
                                     (us_split_fields1 e))))
                   (select (to_array temp___227) (+ r 1)))) (to_rep2
                                                            (let ((temp___228 
                                                            (rec__area_math__ensemble__to
                                                            (us_split_fields1
                                                            e))))
                                                            (select (to_array
                                                                    temp___228) r)))) #x00000001))))
     (forall ((r Int))
     (=>
     (and (<= 1 r)
     (<= r (to_rep (rec__area_math__ensemble__size (us_split_fields1 e)))))
     (bvule (to_rep2
            (let ((temp___229 (rec__area_math__ensemble__from
                              (us_split_fields1 e))))
            (select (to_array temp___229) r))) (to_rep2
                                               (let ((temp___230 (rec__area_math__ensemble__to
                                                                 (us_split_fields1
                                                                 e))))
                                               (select (to_array temp___230) r)))))))) :pattern (
  (is_consistent e)) )))

(declare-const i13s us_rep)

(declare-const attr__ATTRIBUTE_ADDRESS Int)

(declare-fun olt ((_ BitVec 32) us_rep) Bool)

(declare-fun olt__function_guard (Bool (_ BitVec 32) us_rep) Bool)

(define-fun dynamic_invariant2 ((temp___expr_183 (_ BitVec 32))
  (temp___is_init_179 Bool) (temp___skip_constant_180 Bool)
  (temp___do_toplevel_181 Bool) (temp___do_typ_inv_182 Bool)) Bool true)

;; olt__post_axiom
  (assert true)

;; olt__def_axiom
  (assert
  (forall ((b (_ BitVec 32)))
  (forall ((a us_rep))
  (! (= (= (olt b a) true)
     (exists ((r Int))
     (and
     (and (<= 1 r)
     (<= r (to_rep (rec__area_math__ensemble__size (us_split_fields1 a)))))
     (and
     (bvule (to_rep2
            (let ((temp___256 (rec__area_math__ensemble__from
                              (us_split_fields1 a))))
            (select (to_array temp___256) r))) b)
     (bvule b (to_rep2
              (let ((temp___257 (rec__area_math__ensemble__to
                                (us_split_fields1 a))))
              (select (to_array temp___257) r)))))))) :pattern ((olt b a)) ))))

(declare-const e us_rep)

(declare-const attr__ATTRIBUTE_ADDRESS1 Int)

(declare-datatypes ((t55b__ref 0))
(((t55b__refqtmk (t55b__content us_t)))))
(define-fun t55b__ref_t55b__content__projection ((a t55b__ref)) us_t 
  (t55b__content a))

(declare-datatypes ((t56b__ref 0))
(((t56b__refqtmk (t56b__content us_t)))))
(define-fun t56b__ref_t56b__content__projection ((a t56b__ref)) us_t 
  (t56b__content a))

(declare-datatypes ((us_split_fields2 0))
(((us_split_fieldsqtmk1
  (rec__area_math__ensemble__size1 natural)(rec__area_math__ensemble__from1 us_t)(rec__area_math__ensemble__to1 us_t)))))
(define-fun us_split_fields_rec__area_math__ensemble__size__5__projection ((a us_split_fields2)) natural 
  (rec__area_math__ensemble__size1 a))

(define-fun us_split_fields_rec__area_math__ensemble__from__5__projection ((a us_split_fields2)) us_t 
  (rec__area_math__ensemble__from1 a))

(define-fun us_split_fields_rec__area_math__ensemble__to__5__projection ((a us_split_fields2)) us_t 
  (rec__area_math__ensemble__to1 a))

(declare-datatypes ((us_split_fields__ref1 0))
(((us_split_fields__refqtmk1 (us_split_fields__content1 us_split_fields2)))))
(define-fun us_split_fields__ref___split_fields__content__5__projection ((a us_split_fields__ref1)) us_split_fields2 
  (us_split_fields__content1 a))

(declare-datatypes ((us_rep1 0))
(((us_repqtmk1
  (us_split_discrs2 us_split_discrs)(us_split_fields3 us_split_fields2)))))
(define-fun us_rep___split_discrs__5__projection ((a us_rep1)) us_split_discrs 
  (us_split_discrs2 a))

(define-fun us_rep___split_fields__5__projection ((a us_rep1)) us_split_fields2 
  (us_split_fields3 a))

(define-fun to_base ((a us_rep1)) us_rep (us_repqtmk (us_split_discrs2 a)
                                         (us_split_fieldsqtmk
                                         (rec__area_math__ensemble__size1
                                         (us_split_fields3 a))
                                         (let ((temp___176 (rec__area_math__ensemble__from1
                                                           (us_split_fields3
                                                           a))))
                                         (of_array (to_array temp___176)
                                         (first1 temp___176)
                                         (last1 temp___176)))
                                         (let ((temp___178 (rec__area_math__ensemble__to1
                                                           (us_split_fields3
                                                           a))))
                                         (of_array (to_array temp___178)
                                         (first1 temp___178)
                                         (last1 temp___178))))))

(define-fun of_base ((r us_rep)) us_rep1 (us_repqtmk1 (us_split_discrs1 r)
                                         (us_split_fieldsqtmk1
                                         (rec__area_math__ensemble__size
                                         (us_split_fields1 r))
                                         (let ((temp___175 (rec__area_math__ensemble__from
                                                           (us_split_fields1
                                                           r))))
                                         (of_array (to_array temp___175)
                                         (first1 temp___175)
                                         (last1 temp___175)))
                                         (let ((temp___177 (rec__area_math__ensemble__to
                                                           (us_split_fields1
                                                           r))))
                                         (of_array (to_array temp___177)
                                         (first1 temp___177)
                                         (last1 temp___177))))))

(define-fun bool_eq3 ((a us_rep1)
  (b us_rep1)) Bool (ite (and
                         (= (to_rep
                            (rec__area_math__ensemble__max
                            (us_split_discrs2 a))) (to_rep
                                                   (rec__area_math__ensemble__max
                                                   (us_split_discrs2 b))))
                         (and
                         (and
                         (= (to_rep
                            (rec__area_math__ensemble__size1
                            (us_split_fields3 a))) (to_rep
                                                   (rec__area_math__ensemble__size1
                                                   (us_split_fields3 b))))
                         (= (bool_eq1
                            (rec__area_math__ensemble__from1
                            (us_split_fields3 a))
                            (rec__area_math__ensemble__from1
                            (us_split_fields3 b))) true))
                         (= (bool_eq1
                            (rec__area_math__ensemble__to1
                            (us_split_fields3 a))
                            (rec__area_math__ensemble__to1
                            (us_split_fields3 b))) true)))
                    true false))

(declare-const value__size2 Int)

(declare-const object__size2 Int)

(declare-const alignment2 Int)

;; value__size_axiom
  (assert (<= 0 value__size2))

;; object__size_axiom
  (assert (<= 0 object__size2))

;; alignment_axiom
  (assert (<= 0 alignment2))

(declare-const area_math__ensemble__max__first__bit1 Int)

(declare-const area_math__ensemble__max__last__bit1 Int)

(declare-const area_math__ensemble__max__position1 Int)

;; area_math__ensemble__max__first__bit_axiom
  (assert (<= 0 area_math__ensemble__max__first__bit1))

;; area_math__ensemble__max__last__bit_axiom
  (assert
  (< area_math__ensemble__max__first__bit1 area_math__ensemble__max__last__bit1))

;; area_math__ensemble__max__position_axiom
  (assert (<= 0 area_math__ensemble__max__position1))

(declare-const area_math__ensemble__size__first__bit1 Int)

(declare-const area_math__ensemble__size__last__bit1 Int)

(declare-const area_math__ensemble__size__position1 Int)

;; area_math__ensemble__size__first__bit_axiom
  (assert (<= 0 area_math__ensemble__size__first__bit1))

;; area_math__ensemble__size__last__bit_axiom
  (assert
  (< area_math__ensemble__size__first__bit1 area_math__ensemble__size__last__bit1))

;; area_math__ensemble__size__position_axiom
  (assert (<= 0 area_math__ensemble__size__position1))

(declare-const area_math__ensemble__from__first__bit1 Int)

(declare-const area_math__ensemble__from__last__bit1 Int)

(declare-const area_math__ensemble__from__position1 Int)

;; area_math__ensemble__from__first__bit_axiom
  (assert (<= 0 area_math__ensemble__from__first__bit1))

;; area_math__ensemble__from__last__bit_axiom
  (assert
  (< area_math__ensemble__from__first__bit1 area_math__ensemble__from__last__bit1))

;; area_math__ensemble__from__position_axiom
  (assert (<= 0 area_math__ensemble__from__position1))

(declare-const area_math__ensemble__to__first__bit1 Int)

(declare-const area_math__ensemble__to__last__bit1 Int)

(declare-const area_math__ensemble__to__position1 Int)

;; area_math__ensemble__to__first__bit_axiom
  (assert (<= 0 area_math__ensemble__to__first__bit1))

;; area_math__ensemble__to__last__bit_axiom
  (assert
  (< area_math__ensemble__to__first__bit1 area_math__ensemble__to__last__bit1))

;; area_math__ensemble__to__position_axiom
  (assert (<= 0 area_math__ensemble__to__position1))

(declare-fun user_eq5 (us_rep1 us_rep1) Bool)

(declare-const dummy5 us_rep1)

(declare-datatypes ((tresultS__ref 0))
(((tresultS__refqtmk (tresultS__content us_rep1)))))
(define-fun tresultS__ref_tresultS__content__projection ((a tresultS__ref)) us_rep1 
  (tresultS__content a))

(declare-const result____split_discrs us_split_discrs)

(declare-const attr__ATTRIBUTE_ADDRESS2 Int)

(define-fun dynamic_invariant3 ((temp___expr_337 us_rep1)
  (temp___is_init_333 Bool) (temp___skip_constant_334 Bool)
  (temp___do_toplevel_335 Bool)
  (temp___do_typ_inv_336 Bool)) Bool (and
                                     (and
                                     (=>
                                     (not (= temp___skip_constant_334 true))
                                     (in_range3
                                     (+ (to_rep
                                        (rec__area_math__ensemble__size
                                        (us_split_fields1 e))) 1)
                                     (us_split_discrs1
                                     (to_base temp___expr_337))))
                                     (=> (= temp___is_init_333 true)
                                     (and
                                     (<= (to_rep
                                         (rec__area_math__ensemble__size
                                         (us_split_fields1
                                         (to_base temp___expr_337)))) 
                                     (last1
                                     (rec__area_math__ensemble__from
                                     (us_split_fields1
                                     (to_base temp___expr_337)))))
                                     (<= (to_rep
                                         (rec__area_math__ensemble__size
                                         (us_split_fields1
                                         (to_base temp___expr_337)))) 
                                     (last1
                                     (rec__area_math__ensemble__to
                                     (us_split_fields1
                                     (to_base temp___expr_337))))))))
                                     (and
                                     (and (dynamic_property 1
                                     (+ (to_rep
                                        (rec__area_math__ensemble__size
                                        (us_split_fields1 e))) 1)
                                     (first1
                                     (rec__area_math__ensemble__from1
                                     (us_split_fields3 temp___expr_337)))
                                     (last1
                                     (rec__area_math__ensemble__from1
                                     (us_split_fields3 temp___expr_337))))
                                     (and
                                     (= (first1
                                        (rec__area_math__ensemble__from1
                                        (us_split_fields3 temp___expr_337))) 1)
                                     (= (last1
                                        (rec__area_math__ensemble__from1
                                        (us_split_fields3 temp___expr_337))) (+ 
                                     (to_rep
                                     (rec__area_math__ensemble__size
                                     (us_split_fields1 e))) 1))))
                                     (and (dynamic_property 1
                                     (+ (to_rep
                                        (rec__area_math__ensemble__size
                                        (us_split_fields1 e))) 1)
                                     (first1
                                     (rec__area_math__ensemble__to1
                                     (us_split_fields3 temp___expr_337)))
                                     (last1
                                     (rec__area_math__ensemble__to1
                                     (us_split_fields3 temp___expr_337))))
                                     (and
                                     (= (first1
                                        (rec__area_math__ensemble__to1
                                        (us_split_fields3 temp___expr_337))) 1)
                                     (= (last1
                                        (rec__area_math__ensemble__to1
                                        (us_split_fields3 temp___expr_337))) (+ 
                                     (to_rep
                                     (rec__area_math__ensemble__size
                                     (us_split_fields1 e))) 1)))))))

(define-fun default_initial_assumption ((temp___expr_343 us_rep1)
  (temp___skip_top_level_344 Bool)) Bool (and
                                         (= (to_rep
                                            (rec__area_math__ensemble__max
                                            (us_split_discrs2
                                            temp___expr_343))) (+ (to_rep
                                                                  (rec__area_math__ensemble__size
                                                                  (us_split_fields1
                                                                  e))) 1))
                                         (and
                                         (and
                                         (= (first1
                                            (rec__area_math__ensemble__from1
                                            (us_split_fields3
                                            temp___expr_343))) 1)
                                         (= (last1
                                            (rec__area_math__ensemble__from1
                                            (us_split_fields3
                                            temp___expr_343))) (+ (to_rep
                                                                  (rec__area_math__ensemble__size
                                                                  (us_split_fields1
                                                                  e))) 1)))
                                         (and
                                         (= (first1
                                            (rec__area_math__ensemble__to1
                                            (us_split_fields3
                                            temp___expr_343))) 1)
                                         (= (last1
                                            (rec__area_math__ensemble__to1
                                            (us_split_fields3
                                            temp___expr_343))) (+ (to_rep
                                                                  (rec__area_math__ensemble__size
                                                                  (us_split_fields1
                                                                  e))) 1))))))

(define-fun dynamic_predicate1 ((temp___353 us_rep1)) Bool (and
                                                           (<= (to_rep
                                                               (rec__area_math__ensemble__size
                                                               (us_split_fields1
                                                               (to_base
                                                               temp___353)))) 
                                                           (last1
                                                           (rec__area_math__ensemble__from
                                                           (us_split_fields1
                                                           (to_base
                                                           temp___353)))))
                                                           (<= (to_rep
                                                               (rec__area_math__ensemble__size
                                                               (us_split_fields1
                                                               (to_base
                                                               temp___353)))) 
                                                           (last1
                                                           (rec__area_math__ensemble__to
                                                           (us_split_fields1
                                                           (to_base
                                                           temp___353)))))))

(declare-const attr__ATTRIBUTE_ADDRESS3 Int)

(declare-const attr__ATTRIBUTE_ADDRESS4 Int)

(declare-const attr__ATTRIBUTE_ADDRESS5 Int)

(declare-const r61b Int)

(declare-const attr__ATTRIBUTE_ADDRESS6 Int)

(declare-const attr__ATTRIBUTE_ADDRESS7 Int)

(declare-datatypes ((t58b__ref 0))
(((t58b__refqtmk (t58b__content us_t)))))
(define-fun t58b__ref_t58b__content__projection ((a t58b__ref)) us_t 
  (t58b__content a))

(declare-datatypes ((t60b__ref 0))
(((t60b__refqtmk (t60b__content us_t)))))
(define-fun t60b__ref_t60b__content__projection ((a t60b__ref)) us_t 
  (t60b__content a))

(define-fun dynamic_property1 ((first_int Int) (last_int Int)
  (x Int)) Bool (and (<= first_int x) (<= x last_int)))

(declare-fun attr__ATTRIBUTE_IMAGE4 (Int) us_image)

(declare-fun attr__ATTRIBUTE_VALUE__pre_check4 (us_image) Bool)

(declare-fun attr__ATTRIBUTE_VALUE4 (us_image) Int)

(declare-fun user_eq6 (integer integer) Bool)

(declare-const dummy6 integer)

(declare-datatypes ((t62b__ref 0))
(((t62b__refqtmk (t62b__content integer)))))
(define-fun t62b__ref_t62b__content__projection ((a t62b__ref)) integer 
  (t62b__content a))

(declare-fun temp_____aggregate_def_422 ((_ BitVec 32) Int Int) us_t)

(declare-fun temp_____aggregate_def_435 ((_ BitVec 32) Int Int) us_t)

(define-fun dynamic_invariant4 ((temp___expr_18 Int) (temp___is_init_14 Bool)
  (temp___skip_constant_15 Bool) (temp___do_toplevel_16 Bool)
  (temp___do_typ_inv_17 Bool)) Bool (=>
                                    (or (= temp___is_init_14 true)
                                    (<= (- 2147483648) 2147483647))
                                    (in_range1 temp___expr_18)))

;; def_axiom
  (assert
  (forall ((temp___424 (_ BitVec 32)))
  (forall ((temp___425 Int) (temp___426 Int))
  (let ((temp___423 (temp_____aggregate_def_422 temp___424 temp___425
                    temp___426)))
  (=> (dynamic_invariant2 temp___424 true true true true)
  (and
  (=> (dynamic_property (- 2147483648) 2147483647 temp___425 temp___426)
  (and (= (first1 temp___423) temp___425) (= (last1 temp___423) temp___426)))
  (forall ((temp___427 Int))
  (= (to_rep2 (select (to_array temp___423) temp___427)) temp___424))))))))

;; def_axiom
  (assert
  (forall ((temp___437 (_ BitVec 32)))
  (forall ((temp___438 Int) (temp___439 Int))
  (let ((temp___436 (temp_____aggregate_def_435 temp___437 temp___438
                    temp___439)))
  (=> (dynamic_invariant2 temp___437 true true true true)
  (and
  (=> (dynamic_property (- 2147483648) 2147483647 temp___438 temp___439)
  (and (= (first1 temp___436) temp___438) (= (last1 temp___436) temp___439)))
  (forall ((temp___440 Int))
  (= (to_rep2 (select (to_array temp___436) temp___440)) temp___437))))))))

(define-fun dynamic_invariant5 ((temp___expr_190 us_t)
  (temp___is_init_186 Bool) (temp___skip_constant_187 Bool)
  (temp___do_toplevel_188 Bool)
  (temp___do_typ_inv_189 Bool)) Bool (=>
                                     (not (= temp___skip_constant_187 true))
                                     (dynamic_property (- 2147483648)
                                     2147483647 (first1 temp___expr_190)
                                     (last1 temp___expr_190))))

;; r61b__def_axiom
  (assert
  (= r61b (to_rep (rec__area_math__ensemble__size (us_split_fields1 e)))))

(declare-const rliteral natural)

;; rliteral_axiom
  (assert (= (naturalqtint rliteral) 0))

(assert
;; defqtvc
 ;; File "area_math.ads", line 46, characters 0-0
  (not
  (forall ((result____split_fields us_split_fields2) (o Int))
  (=> (dynamic_invariant1 i13s true false true true)
  (=> (dynamic_invariant1 i13s true false true true)
  (=> (dynamic_invariant1 e true false true true)
  (=>
  (< 0 (- 2147483647 (to_rep
                     (rec__area_math__ensemble__max (us_split_discrs1 e)))))
  (=> (is_consistent__function_guard (is_consistent e) e)
  (=> (= (is_consistent e) true)
  (=> (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) o)
  (let ((o1 (+ o 1)))
  (=> (in_range1 o1)
  (=> (in_range2 o1)
  (=> (default_initial_assumption
  (us_repqtmk1 result____split_discrs result____split_fields) false)
  (=> (dynamic_invariant3
  (us_repqtmk1 result____split_discrs result____split_fields) false false
  true true)
  (forall ((prev (_ BitVec 32)))
  (=> (= prev #x00000000)
  (=> (dynamic_invariant2 prev true false true true)
  (forall ((next (_ BitVec 32)))
  (=> (= next #x00000000)
  (=> (dynamic_invariant2 next true false true true)
  (=> (dynamic_predicate1
  (us_repqtmk1 result____split_discrs
  (us_split_fieldsqtmk1 rliteral
  (rec__area_math__ensemble__from1 result____split_fields)
  (rec__area_math__ensemble__to1 result____split_fields))))
  (forall ((result____split_fields1 us_split_fields2))
  (=>
  (= result____split_fields1 (us_split_fieldsqtmk1 rliteral
                             (rec__area_math__ensemble__from1
                             result____split_fields)
                             (rec__area_math__ensemble__to1
                             result____split_fields)))
  (forall ((o2 Int))
  (=> (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) o2)
  (let ((temp___428 (temp_____aggregate_def_422 #x00000000 1 (+ o2 1))))
  (=> (= (first1 temp___428) 1)
  (=>
  (= (last1 temp___428) (+ (to_rep
                           (rec__area_math__ensemble__size
                           (us_split_fields1 e))) 1))
  (=>
  (ite (<= (first1 temp___428) (last1 temp___428))
  (and
  (<= 1 (+ (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) 1))
  (= (- (last1 temp___428) (first1 temp___428)) (- (+ (to_rep
                                                      (rec__area_math__ensemble__size
                                                      (us_split_fields1 e))) 1) 1)))
  (< (+ (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) 1) 1))
  (let ((temp___430 (of_array (to_array temp___428) (first1 temp___428)
                    (last1 temp___428))))
  (let ((temp___431 (rec__area_math__ensemble__from1 result____split_fields1)))
  (=>
  (ite (<= (first1 temp___430) (last1 temp___430))
  (and (<= (first1 temp___431) (last1 temp___431))
  (= (- (last1 temp___430) (first1 temp___430)) (- (last1 temp___431) 
  (first1 temp___431)))) (< (last1 temp___431) (first1 temp___431)))
  (=> (dynamic_predicate1
  (us_repqtmk1 result____split_discrs
  (us_split_fieldsqtmk1
  (rec__area_math__ensemble__size1 result____split_fields1) temp___430
  (rec__area_math__ensemble__to1 result____split_fields1))))
  (forall ((result____split_fields2 us_split_fields2))
  (=>
  (= result____split_fields2 (us_split_fieldsqtmk1
                             (rec__area_math__ensemble__size1
                             result____split_fields1) temp___430
                             (rec__area_math__ensemble__to1
                             result____split_fields1)))
  (forall ((o3 Int))
  (=> (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) o3)
  (let ((temp___441 (temp_____aggregate_def_435 #x00000000 1 (+ o3 1))))
  (=> (= (first1 temp___441) 1)
  (=>
  (= (last1 temp___441) (+ (to_rep
                           (rec__area_math__ensemble__size
                           (us_split_fields1 e))) 1))
  (=>
  (ite (<= (first1 temp___441) (last1 temp___441))
  (and
  (<= 1 (+ (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) 1))
  (= (- (last1 temp___441) (first1 temp___441)) (- (+ (to_rep
                                                      (rec__area_math__ensemble__size
                                                      (us_split_fields1 e))) 1) 1)))
  (< (+ (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) 1) 1))
  (let ((temp___443 (of_array (to_array temp___441) (first1 temp___441)
                    (last1 temp___441))))
  (let ((temp___444 (rec__area_math__ensemble__to1 result____split_fields2)))
  (=>
  (ite (<= (first1 temp___443) (last1 temp___443))
  (and (<= (first1 temp___444) (last1 temp___444))
  (= (- (last1 temp___443) (first1 temp___443)) (- (last1 temp___444) 
  (first1 temp___444)))) (< (last1 temp___444) (first1 temp___444)))
  (=> (dynamic_predicate1
  (us_repqtmk1 result____split_discrs
  (us_split_fieldsqtmk1
  (rec__area_math__ensemble__size1 result____split_fields2)
  (rec__area_math__ensemble__from1 result____split_fields2) temp___443)))
  (forall ((result____split_fields3 us_split_fields2))
  (=>
  (= result____split_fields3 (us_split_fieldsqtmk1
                             (rec__area_math__ensemble__size1
                             result____split_fields2)
                             (rec__area_math__ensemble__from1
                             result____split_fields2) temp___443))
  (forall ((area_math__Onot__L_7__R61b__assume Int))
  (=>
  (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) area_math__Onot__L_7__R61b__assume)
  (=> (= area_math__Onot__L_7__R61b__assume r61b)
  (=> (dynamic_invariant r61b true false true true)
  (let ((temp___449 r61b))
  (forall ((i Int))
  (=> (= i 0)
  (=>
  (= (and (ite (<= 0 i) true false) (ite (<= i temp___449) true false)) true)
  (forall ((skip Bool))
  (=> (= skip (distinct 0 0))
  (forall ((spark__branch Bool) (prev1 (_ BitVec 32)) (skip1 Bool))
  (=>
  (exists ((spark__branch1 Bool))
  (and (= spark__branch1 (ite (= i 0) true false))
  (ite (= spark__branch1 true)
  (exists ((o4 Bool))
  (and
  (exists ((o5 Int))
  (and (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) o5)
  (ite (not (= o5 0))
  (let ((temp___450 (rec__area_math__ensemble__from (us_split_fields1 e))))
  (let ((o6 (+ i 1)))
  (and (in_range1 o6)
  (and (and (<= (first1 temp___450) o6) (<= o6 (last1 temp___450)))
  (exists ((o7 (_ BitVec 32)))
  (and (= (to_rep2 (select (to_array temp___450) o6)) o7)
  (= o4 (ite (= o7 #x00000000) true false)))))))) (= o4 false))))
  (and (= spark__branch o4)
  (ite (= spark__branch true) (and (= skip1 (distinct 1 0)) (= prev1 prev))
  (and (= prev1 #x00000000) (= skip1 skip))))))
  (and
  (and
  (let ((temp___452 (rec__area_math__ensemble__to (us_split_fields1 e))))
  (and (and (<= (first1 temp___452) i) (<= i (last1 temp___452)))
  (exists ((o4 (_ BitVec 32)))
  (and (= (to_rep2 (select (to_array temp___452) i)) o4)
  (= prev1 (bvadd o4 #x00000001)))))) (= spark__branch spark__branch1))
  (= skip1 skip)))))
  (forall ((spark__branch1 Bool) (next1 (_ BitVec 32)) (skip2 Bool))
  (=>
  (exists ((o4 Int))
  (and (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) o4)
  (exists ((spark__branch2 Bool))
  (and (= spark__branch2 (ite (= i o4) true false))
  (ite (= spark__branch2 true)
  (exists ((o5 Bool))
  (and
  (exists ((o6 Int))
  (and (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) o6)
  (ite (not (= o6 0))
  (let ((temp___453 (rec__area_math__ensemble__to (us_split_fields1 e))))
  (and (and (<= (first1 temp___453) i) (<= i (last1 temp___453)))
  (exists ((o7 (_ BitVec 32)))
  (and (= (to_rep2 (select (to_array temp___453) i)) o7)
  (= o5 (ite (= o7 #xFFFFFFFF) true false)))))) (= o5 false))))
  (and (= spark__branch1 o5)
  (ite (= spark__branch1 true) (and (= skip2 (distinct 1 0)) (= next1 next))
  (and (= next1 #xFFFFFFFF) (= skip2 skip1))))))
  (and
  (and
  (let ((temp___454 (rec__area_math__ensemble__from (us_split_fields1 e))))
  (let ((o5 (+ i 1)))
  (and (in_range1 o5)
  (and (and (<= (first1 temp___454) o5) (<= o5 (last1 temp___454)))
  (exists ((o6 (_ BitVec 32)))
  (and (= (to_rep2 (select (to_array temp___454) o5)) o6)
  (= next1 (bvsub o6 #x00000001)))))))) (= spark__branch1 spark__branch2))
  (= skip2 skip1)))))))
  (forall ((spark__branch2 Bool) (result____split_fields4 us_split_fields2))
  (=>
  (forall ((result____split_fields5 us_split_fields2) (b (_ BitVec 32)))
  (olt__function_guard
  (olt b
  (to_base (us_repqtmk1 result____split_discrs result____split_fields5))) b
  (to_base (us_repqtmk1 result____split_discrs result____split_fields5))))
  (=>
  (and (= spark__branch2 (ite (= skip2 true) false true))
  (ite (= spark__branch2 true)
  (and (bvule prev1 next1)
  (and
  (=>
  (< 0 (to_rep (rec__area_math__ensemble__size1 result____split_fields3)))
  (bvugt prev1 (to_rep2
               (select (to_array
                       (rec__area_math__ensemble__to1
                       result____split_fields3)) (to_rep
                                                 (rec__area_math__ensemble__size1
                                                 result____split_fields3))))))
  (exists ((o4 Int))
  (and
  (= (to_rep (rec__area_math__ensemble__size1 result____split_fields3)) o4)
  (let ((o5 (+ o4 1)))
  (and (in_range1 o5)
  (exists ((o6 natural))
  (and (= (to_rep o6) o5)
  (and (dynamic_predicate1
  (us_repqtmk1 result____split_discrs
  (us_split_fieldsqtmk1 o6
  (rec__area_math__ensemble__from1 result____split_fields3)
  (rec__area_math__ensemble__to1 result____split_fields3))))
  (exists ((result____split_fields5 us_split_fields2))
  (and
  (= result____split_fields5 (us_split_fieldsqtmk1 o6
                             (rec__area_math__ensemble__from1
                             result____split_fields3)
                             (rec__area_math__ensemble__to1
                             result____split_fields3)))
  (let ((temp___462 (rec__area_math__ensemble__from1 result____split_fields5)))
  (exists ((o7 address_type))
  (and (= (to_rep2 o7) prev1)
  (exists ((temp___463 Int))
  (and
  (= (to_rep (rec__area_math__ensemble__size1 result____split_fields5)) temp___463)
  (and
  (and (<= (first1 temp___462) temp___463)
  (<= temp___463 (last1 temp___462)))
  (and (dynamic_predicate1
  (us_repqtmk1 result____split_discrs
  (us_split_fieldsqtmk1
  (rec__area_math__ensemble__size1 result____split_fields5)
  (us_tqtmk
  (store (to_array (rec__area_math__ensemble__from1 result____split_fields5)) temp___463 o7)
  (rt (rec__area_math__ensemble__from1 result____split_fields5)))
  (rec__area_math__ensemble__to1 result____split_fields5))))
  (exists ((result____split_fields6 us_split_fields2))
  (and
  (= result____split_fields6 (us_split_fieldsqtmk1
                             (rec__area_math__ensemble__size1
                             result____split_fields5)
                             (us_tqtmk
                             (store (to_array
                                    (rec__area_math__ensemble__from1
                                    result____split_fields5)) temp___463 o7)
                             (rt
                             (rec__area_math__ensemble__from1
                             result____split_fields5)))
                             (rec__area_math__ensemble__to1
                             result____split_fields5)))
  (let ((temp___467 (rec__area_math__ensemble__to1 result____split_fields6)))
  (exists ((o8 address_type))
  (and (= (to_rep2 o8) next1)
  (exists ((temp___468 Int))
  (and
  (= (to_rep (rec__area_math__ensemble__size1 result____split_fields6)) temp___468)
  (and
  (and (<= (first1 temp___467) temp___468)
  (<= temp___468 (last1 temp___467)))
  (and (dynamic_predicate1
  (us_repqtmk1 result____split_discrs
  (us_split_fieldsqtmk1
  (rec__area_math__ensemble__size1 result____split_fields6)
  (rec__area_math__ensemble__from1 result____split_fields6)
  (us_tqtmk
  (store (to_array (rec__area_math__ensemble__to1 result____split_fields6)) temp___468 o8)
  (rt (rec__area_math__ensemble__to1 result____split_fields6))))))
  (and
  (= result____split_fields4 (us_split_fieldsqtmk1
                             (rec__area_math__ensemble__size1
                             result____split_fields6)
                             (rec__area_math__ensemble__from1
                             result____split_fields6)
                             (us_tqtmk
                             (store (to_array
                                    (rec__area_math__ensemble__to1
                                    result____split_fields6)) temp___468 o8)
                             (rt
                             (rec__area_math__ensemble__to1
                             result____split_fields6)))))
  (and
  (forall ((b (_ BitVec 32)))
  (=> (and (bvule prev1 b) (bvule b next1))
  (= (olt b
     (to_base (us_repqtmk1 result____split_discrs result____split_fields4))) true)))
  (and
  (or
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__from1 result____split_fields4)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields4)))) #x00000000)
  (and (not (= i 0))
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__from1 result____split_fields4)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields4)))) (bvadd 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__to (us_split_fields1 e))) i)) #x00000001))))
  (or
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__to1 result____split_fields4)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields4)))) #xFFFFFFFF)
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__to1 result____split_fields4)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields4)))) (bvsub 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__from (us_split_fields1 e))) (+ i 1))) #x00000001)))))))))))))))))))))))))))))))))
  (= result____split_fields4 result____split_fields3)))
  (=> (in_range1 (+ i 1))
  (forall ((o4 Int))
  (=>
  (= (to_rep (rec__area_math__ensemble__size1 result____split_fields4)) o4)
  (forall ((result____split_fields5 us_split_fields2) (prev2 (_ BitVec 32))
  (next2 (_ BitVec 32)) (skip3 Bool) (i1 Int))
  (=> (is_consistent__function_guard (is_consistent e) e)
  (=>
  (forall ((result____split_fields6 us_split_fields2))
  (is_consistent__function_guard
  (is_consistent
  (to_base (us_repqtmk1 result____split_discrs result____split_fields6)))
  (to_base (us_repqtmk1 result____split_discrs result____split_fields6))))
  (=>
  (and
  (<= (to_rep (rec__area_math__ensemble__size1 result____split_fields5)) (+ i1 1))
  (and
  (=> (not (= skip3 true))
  (< 0 (to_rep (rec__area_math__ensemble__size1 result____split_fields5))))
  (and
  (=> (not (= skip3 true))
  (or
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__from1 result____split_fields5)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields5)))) #x00000000)
  (and (not (= i1 0))
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__from1 result____split_fields5)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields5)))) (bvadd 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__to (us_split_fields1 e))) i1)) #x00000001)))))
  (and
  (=> (not (= skip3 true))
  (or
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__to1 result____split_fields5)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields5)))) #xFFFFFFFF)
  (and (< i1 (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))))
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__to1 result____split_fields5)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields5)))) (bvsub 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__from (us_split_fields1 e))) (+ i1 1))) #x00000001)))))
  (and
  (=> (= skip3 true)
  (or
  (and
  (and (= i1 0)
  (not (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) 0)))
  (= (to_rep2
     (select (to_array (rec__area_math__ensemble__from (us_split_fields1 e))) (+ i1 1))) #x00000000))
  (and
  (and (= i1 (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))))
  (not (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) 0)))
  (= (to_rep2
     (select (to_array (rec__area_math__ensemble__to (us_split_fields1 e))) i1)) #xFFFFFFFF))))
  (and (= (is_consistent e) true)
  (and
  (=>
  (< 0 (to_rep (rec__area_math__ensemble__size1 result____split_fields5)))
  (ite (and
       (< i1 (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))))
       (< 0 (to_rep (rec__area_math__ensemble__size (us_split_fields1 e)))))
  (and
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__to1 result____split_fields5)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields5)))) (bvsub 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__from (us_split_fields1 e))) (+ i1 1))) #x00000001))
  (bvult (to_rep2
         (select (to_array
                 (rec__area_math__ensemble__to1 result____split_fields5)) 
         (to_rep (rec__area_math__ensemble__size1 result____split_fields5)))) (bvadd 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__to (us_split_fields1 e))) (+ i1 1))) #x00000001)))
  (= i1 (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))))))
  (= (is_consistent
     (to_base (us_repqtmk1 result____split_discrs result____split_fields5))) true))))))))
  (=>
  (= (and (ite (and
               (and
               (and (dynamic_invariant2 prev2 true true true true)
               (dynamic_property1 0 r61b i1)) (dynamic_invariant2 next2 true
               true true true)) (dynamic_invariant3
               (us_repqtmk1 result____split_discrs result____split_fields5)
               false true true true))
          true false) (ite (and (<= 0 i1) (<= i1 r61b)) true false)) true)
  (=> (not (= i1 temp___449))
  (forall ((i2 Int))
  (=> (= i2 (+ i1 1))
  (forall ((skip4 Bool))
  (=> (= skip4 (distinct 0 0))
  (forall ((spark__branch3 Bool) (prev3 (_ BitVec 32)) (skip5 Bool))
  (=>
  (exists ((spark__branch4 Bool))
  (and (= spark__branch4 (ite (= i2 0) true false))
  (ite (= spark__branch4 true)
  (exists ((o5 Bool))
  (and
  (exists ((o6 Int))
  (and (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) o6)
  (ite (not (= o6 0))
  (let ((temp___450 (rec__area_math__ensemble__from (us_split_fields1 e))))
  (let ((o7 (+ i2 1)))
  (and (in_range1 o7)
  (and (and (<= (first1 temp___450) o7) (<= o7 (last1 temp___450)))
  (exists ((o8 (_ BitVec 32)))
  (and (= (to_rep2 (select (to_array temp___450) o7)) o8)
  (= o5 (ite (= o8 #x00000000) true false)))))))) (= o5 false))))
  (and (= spark__branch3 o5)
  (ite (= spark__branch3 true) (and (= skip5 (distinct 1 0)) (= prev3 prev2))
  (and (= prev3 #x00000000) (= skip5 skip4))))))
  (and
  (and
  (let ((temp___452 (rec__area_math__ensemble__to (us_split_fields1 e))))
  (and (and (<= (first1 temp___452) i2) (<= i2 (last1 temp___452)))
  (exists ((o5 (_ BitVec 32)))
  (and (= (to_rep2 (select (to_array temp___452) i2)) o5)
  (= prev3 (bvadd o5 #x00000001)))))) (= spark__branch3 spark__branch4))
  (= skip5 skip4)))))
  (forall ((spark__branch4 Bool) (next3 (_ BitVec 32)) (skip6 Bool))
  (=>
  (exists ((o5 Int))
  (and (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) o5)
  (exists ((spark__branch5 Bool))
  (and (= spark__branch5 (ite (= i2 o5) true false))
  (ite (= spark__branch5 true)
  (exists ((o6 Bool))
  (and
  (exists ((o7 Int))
  (and (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) o7)
  (ite (not (= o7 0))
  (let ((temp___453 (rec__area_math__ensemble__to (us_split_fields1 e))))
  (and (and (<= (first1 temp___453) i2) (<= i2 (last1 temp___453)))
  (exists ((o8 (_ BitVec 32)))
  (and (= (to_rep2 (select (to_array temp___453) i2)) o8)
  (= o6 (ite (= o8 #xFFFFFFFF) true false)))))) (= o6 false))))
  (and (= spark__branch4 o6)
  (ite (= spark__branch4 true) (and (= skip6 (distinct 1 0)) (= next3 next2))
  (and (= next3 #xFFFFFFFF) (= skip6 skip5))))))
  (and
  (and
  (let ((temp___454 (rec__area_math__ensemble__from (us_split_fields1 e))))
  (let ((o6 (+ i2 1)))
  (and (in_range1 o6)
  (and (and (<= (first1 temp___454) o6) (<= o6 (last1 temp___454)))
  (exists ((o7 (_ BitVec 32)))
  (and (= (to_rep2 (select (to_array temp___454) o6)) o7)
  (= next3 (bvsub o7 #x00000001)))))))) (= spark__branch4 spark__branch5))
  (= skip6 skip5)))))))
  (forall ((result____split_fields6 us_split_fields2))
  (=>
  (forall ((result____split_fields7 us_split_fields2) (b (_ BitVec 32)))
  (olt__function_guard
  (olt b
  (to_base (us_repqtmk1 result____split_discrs result____split_fields7))) b
  (to_base (us_repqtmk1 result____split_discrs result____split_fields7))))
  (=>
  (exists ((spark__branch5 Bool))
  (and (= spark__branch5 (ite (= skip6 true) false true))
  (ite (= spark__branch5 true)
  (and (bvule prev3 next3)
  (and
  (=>
  (< 0 (to_rep (rec__area_math__ensemble__size1 result____split_fields5)))
  (bvugt prev3 (to_rep2
               (select (to_array
                       (rec__area_math__ensemble__to1
                       result____split_fields5)) (to_rep
                                                 (rec__area_math__ensemble__size1
                                                 result____split_fields5))))))
  (exists ((o5 Int))
  (and
  (= (to_rep (rec__area_math__ensemble__size1 result____split_fields5)) o5)
  (let ((o6 (+ o5 1)))
  (and (in_range1 o6)
  (exists ((o7 natural))
  (and (= (to_rep o7) o6)
  (and (dynamic_predicate1
  (us_repqtmk1 result____split_discrs
  (us_split_fieldsqtmk1 o7
  (rec__area_math__ensemble__from1 result____split_fields5)
  (rec__area_math__ensemble__to1 result____split_fields5))))
  (exists ((result____split_fields7 us_split_fields2))
  (and
  (= result____split_fields7 (us_split_fieldsqtmk1 o7
                             (rec__area_math__ensemble__from1
                             result____split_fields5)
                             (rec__area_math__ensemble__to1
                             result____split_fields5)))
  (let ((temp___462 (rec__area_math__ensemble__from1 result____split_fields7)))
  (exists ((o8 address_type))
  (and (= (to_rep2 o8) prev3)
  (exists ((temp___463 Int))
  (and
  (= (to_rep (rec__area_math__ensemble__size1 result____split_fields7)) temp___463)
  (and
  (and (<= (first1 temp___462) temp___463)
  (<= temp___463 (last1 temp___462)))
  (and (dynamic_predicate1
  (us_repqtmk1 result____split_discrs
  (us_split_fieldsqtmk1
  (rec__area_math__ensemble__size1 result____split_fields7)
  (us_tqtmk
  (store (to_array (rec__area_math__ensemble__from1 result____split_fields7)) temp___463 o8)
  (rt (rec__area_math__ensemble__from1 result____split_fields7)))
  (rec__area_math__ensemble__to1 result____split_fields7))))
  (exists ((result____split_fields8 us_split_fields2))
  (and
  (= result____split_fields8 (us_split_fieldsqtmk1
                             (rec__area_math__ensemble__size1
                             result____split_fields7)
                             (us_tqtmk
                             (store (to_array
                                    (rec__area_math__ensemble__from1
                                    result____split_fields7)) temp___463 o8)
                             (rt
                             (rec__area_math__ensemble__from1
                             result____split_fields7)))
                             (rec__area_math__ensemble__to1
                             result____split_fields7)))
  (let ((temp___467 (rec__area_math__ensemble__to1 result____split_fields8)))
  (exists ((o9 address_type))
  (and (= (to_rep2 o9) next3)
  (exists ((temp___468 Int))
  (and
  (= (to_rep (rec__area_math__ensemble__size1 result____split_fields8)) temp___468)
  (and
  (and (<= (first1 temp___467) temp___468)
  (<= temp___468 (last1 temp___467)))
  (and (dynamic_predicate1
  (us_repqtmk1 result____split_discrs
  (us_split_fieldsqtmk1
  (rec__area_math__ensemble__size1 result____split_fields8)
  (rec__area_math__ensemble__from1 result____split_fields8)
  (us_tqtmk
  (store (to_array (rec__area_math__ensemble__to1 result____split_fields8)) temp___468 o9)
  (rt (rec__area_math__ensemble__to1 result____split_fields8))))))
  (and
  (= result____split_fields6 (us_split_fieldsqtmk1
                             (rec__area_math__ensemble__size1
                             result____split_fields8)
                             (rec__area_math__ensemble__from1
                             result____split_fields8)
                             (us_tqtmk
                             (store (to_array
                                    (rec__area_math__ensemble__to1
                                    result____split_fields8)) temp___468 o9)
                             (rt
                             (rec__area_math__ensemble__to1
                             result____split_fields8)))))
  (and
  (forall ((b (_ BitVec 32)))
  (=> (and (bvule prev3 b) (bvule b next3))
  (= (olt b
     (to_base (us_repqtmk1 result____split_discrs result____split_fields6))) true)))
  (and
  (or
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__from1 result____split_fields6)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))) #x00000000)
  (and (not (= i2 0))
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__from1 result____split_fields6)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))) (bvadd 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__to (us_split_fields1 e))) i2)) #x00000001))))
  (or
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__to1 result____split_fields6)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))) #xFFFFFFFF)
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__to1 result____split_fields6)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))) (bvsub 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__from (us_split_fields1 e))) (+ i2 1))) #x00000001)))))))))))))))))))))))))))))))))
  (= result____split_fields6 result____split_fields5))))
  (=> (in_range1 (+ i2 1))
  (forall ((o5 Int))
  (=>
  (= (to_rep (rec__area_math__ensemble__size1 result____split_fields6)) o5)
  (=>
  (<= (to_rep (rec__area_math__ensemble__size1 result____split_fields6)) (+ i2 1))
  (=>
  (=> (not (= skip6 true))
  (< 0 (to_rep (rec__area_math__ensemble__size1 result____split_fields6))))
  (=>
  (=> (not (= skip6 true))
  (or
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__from1 result____split_fields6)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))) #x00000000)
  (and (not (= i2 0))
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__from1 result____split_fields6)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))) (bvadd 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__to (us_split_fields1 e))) i2)) #x00000001)))))
  (=>
  (=> (not (= skip6 true))
  (or
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__to1 result____split_fields6)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))) #xFFFFFFFF)
  (and (< i2 (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))))
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__to1 result____split_fields6)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))) (bvsub 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__from (us_split_fields1 e))) (+ i2 1))) #x00000001)))))
  (=>
  (=> (= skip6 true)
  (or
  (and
  (and (= i2 0)
  (not (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) 0)))
  (= (to_rep2
     (select (to_array (rec__area_math__ensemble__from (us_split_fields1 e))) (+ i2 1))) #x00000000))
  (and
  (and (= i2 (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))))
  (not (= (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))) 0)))
  (= (to_rep2
     (select (to_array (rec__area_math__ensemble__to (us_split_fields1 e))) i2)) #xFFFFFFFF))))
  (=> (is_consistent__function_guard (is_consistent e) e)
  (=> (= (is_consistent e) true)
  (=>
  (=>
  (< 0 (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))
  (ite (and
       (< i2 (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))))
       (< 0 (to_rep (rec__area_math__ensemble__size (us_split_fields1 e)))))
  (and
  (= (to_rep2
     (select (to_array
             (rec__area_math__ensemble__to1 result____split_fields6)) 
     (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))) (bvsub 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__from (us_split_fields1 e))) (+ i2 1))) #x00000001))
  (bvult (to_rep2
         (select (to_array
                 (rec__area_math__ensemble__to1 result____split_fields6)) 
         (to_rep (rec__area_math__ensemble__size1 result____split_fields6)))) (bvadd 
  (to_rep2
  (select (to_array (rec__area_math__ensemble__to (us_split_fields1 e))) (+ i2 1))) #x00000001)))
  (= i2 (to_rep (rec__area_math__ensemble__size (us_split_fields1 e))))))
  (=>
  (forall ((result____split_fields7 us_split_fields2))
  (is_consistent__function_guard
  (is_consistent
  (to_base (us_repqtmk1 result____split_discrs result____split_fields7)))
  (to_base (us_repqtmk1 result____split_discrs result____split_fields7))))
  (= (is_consistent
     (to_base (us_repqtmk1 result____split_discrs result____split_fields6))) true))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))))
(check-sat)
(exit)
