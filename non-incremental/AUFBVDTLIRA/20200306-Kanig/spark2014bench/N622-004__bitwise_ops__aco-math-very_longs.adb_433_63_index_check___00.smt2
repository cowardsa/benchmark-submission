...
;; defqtvc
 ;; File "aco-math-very_longs.adb", line 415, characters 0-0
  ...
  (=> (= (to_rep2 (select (to_array u__split_fields) temp___1612)) o7)
  (let ((temp___1610 (- n 2)))
  (=>
  (and (<= (first1 v__split_fields) temp___1610)
  (<= temp___1610 (last1 v__split_fields)))
  (forall ((o8 (_ BitVec 8)))
  (=> (= (to_rep2 (select (to_array v__split_fields) temp___1610)) o8)
  ...
  (=> (= q_hat2 (bvsub q_hat1 #x0001)) (<= (first1 v__split_fields) (- n 1)))))))))))))))))))))))))))))))))))))))))))))))))))))))
(check-sat)
(exit)
