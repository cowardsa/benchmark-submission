...
(declare-fun lemma5 (tuple0) Bool)

(declare-fun lemma5__function_guard (Bool tuple0) Bool)
...
;; lemma5__def_axiom
  ...
  (! (= (= (lemma5 us_void_param) true)
     ...
     (=> (and (<= 1 n) (<= n 63))
     (=>
     (or (bvugt (shift_right val__ (- n 1)) #x0000000000000000)
     (bvugt (shift_right val__ n) #x0000000000000000))
     (bvugt (shift_right val__ (- n 1)) (shift_right val__ n)))))))) :pattern (
  (lemma5 us_void_param)) )))

(assert
;; defqtvc
 ;; File "bits_manipulation-functions.ads", line 190, characters 0-0
  (not
  (forall
  ((bits_manipulation_unsigned__unsigned_64__functions__lemmas__shift_right__lemma5__result Bool))
  (not
  (=>
  (= bits_manipulation_unsigned__unsigned_64__functions__lemmas__shift_right__lemma5__result true)
  (= (lemma5 Tuple0) true))))))
(check-sat)
(exit)
