...
(declare-fun lemma3 (tuple0) Bool)

(declare-fun lemma3__function_guard (Bool tuple0) Bool)
...
;; lemma3__def_axiom
  ...
  (! (= (= (lemma3 us_void_param) true)
     ...
     (forall ((j Int))
     (=> (and (<= n j) (<= j 15)) (bvule (shift_right val__ j) #x0001))))
     (= (shift_right val__ n) #x0001)))))))) :pattern ((lemma3 us_void_param)) )))

(assert
;; defqtvc
 ;; File "bits_manipulation-functions.ads", line 156, characters 0-0
  (not
  (forall
  ((bits_manipulation_unsigned__unsigned_16__functions__lemmas__shift_right__lemma3__result Bool))
  (not
  (=>
  (= bits_manipulation_unsigned__unsigned_16__functions__lemmas__shift_right__lemma3__result true)
  (= (lemma3 Tuple0) true))))))
(check-sat)
(exit)
