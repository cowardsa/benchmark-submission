...
(declare-fun lemma4 (tuple0) Bool)

(declare-fun lemma4__function_guard (Bool tuple0) Bool)
...
;; lemma4__def_axiom
  ...
  (! (= (= (lemma4 us_void_param) true)
     (forall ((val__ (_ BitVec 32)))
     (=> (and (bvule #x00000000 val__) (bvule val__ #xFFFFFFFF))
     (forall ((n Int))
     (=> (and (<= 1 n) (<= n 31))
     (bvuge (shift_right val__ (- n 1)) (shift_right val__ n))))))) :pattern (
  (lemma4 us_void_param)) )))

(assert
;; defqtvc
 ;; File "bits_manipulation-functions.ads", line 179, characters 0-0
  (not
  (forall
  ((bits_manipulation_unsigned__unsigned_32__functions__lemmas__shift_right__lemma4__result Bool))
  (not
  (=>
  (= bits_manipulation_unsigned__unsigned_32__functions__lemmas__shift_right__lemma4__result true)
  (= (lemma4 Tuple0) true))))))
(check-sat)
(exit)
