...
(assert (forall ((i$ Poly)) (! (=> (has_type i$ INT) (=> (and (<= 0 (%I i$)) (< (%I i$) (vstd!seq.Seq.len.? A&. A& s!))) (%B (%%apply%%0 pred! (vstd!seq.Seq.index.? A&. A& s! i$))))) :pattern ((vstd!seq.Seq.index.? A&. A& s! i$)))))
...
(assert (not (=> (= decrease%init0 (vstd!seq.Seq.len.? A&. A& s!)) (=> (fuel_bool fuel%vstd!seq_lib.impl&%0.filter.) (or (and (=> (not (= (vstd!seq.Seq.len.? A&. A& s!) 0)) (=> (= subseq@ (vstd!seq_lib.impl&%0.drop_last.? A&. A& s!)) (and (=> %%location_label%%0 (check_decrease_int (let ((s!$0 subseq@) (pred!$1 pred!)) (vstd!seq.Seq.len.? A&. A& s!$0)) decrease%init0 false)) (and (=> %%location_label%%1 (req%lib!verus_extra.seq_lib_v.lemma_if_everything_in_seq_satisfies_filter_then_filter_is_identity. A&. A& subseq@ pred!)) (=> (ens%lib!verus_extra.seq_lib_v.lemma_if_everything_in_seq_satisfies_filter_then_filter_is_identity. A&. A& subseq@ pred!) (=> (= s1@ s!) (=> (= s2@ (vstd!seq.Seq.push.? A&. A& subseq@ (vstd!seq.Seq.last.? A&. A& s!))) (and (=> (= tmp%1 (= (vstd!seq.Seq.len.? A&. A& s1@) (vstd!seq.Seq.len.? A&. A& s2@))) (and (=> %%location_label%%2 tmp%1) (=> tmp%1 (and (=> (has_type idx@ INT) (=> (and (<= 0 (%I idx@)) (< (%I idx@) (vstd!seq.Seq.len.? A&. A& s1@))) (=> %%location_label%%3 (= (vstd!seq.Seq.index.? A&. A& s1@ idx@) (vstd!seq.Seq.index.? A&. A& s2@ idx@))))) (=> (forall ((idx$ Poly)) (! (=> (has_type idx$ INT) (=> (and (<= 0 (%I idx$)) (< (%I idx$) (vstd!seq.Seq.len.? A&. A& s1@))) (= (vstd!seq.Seq.index.? A&. A& s1@ idx$) (vstd!seq.Seq.index.? A&. A& s2@ idx$)))) :pattern ((vstd!seq.Seq.index.? A&. A& s1@ idx$)) :pattern ((vstd!seq.Seq.index.? A&. A& s2@ idx$)))) (=> (= tmp%2 (ext_eq false (TYPE%vstd!seq.Seq. A&. A&) s1@ s2@)) (and (=> %%location_label%%4 tmp%2) (=> tmp%2 (=> %%location_label%%5 (= s1@ s2@)))))))))) (=> (= s1@ s2@) %%switch_label%%0))))))))) (=> (not (not (= (vstd!seq.Seq.len.? A&. A& s!) 0))) %%switch_label%%0)) (and (not %%switch_label%%0) (=> %%location_label%%6 (= (vstd!seq_lib.impl&%0.filter.? A&. A& s! (Poly%fun%1. pred!)) s!))))))))
(check-sat)
(exit)
