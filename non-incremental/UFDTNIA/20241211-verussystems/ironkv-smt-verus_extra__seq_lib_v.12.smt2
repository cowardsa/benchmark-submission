...
(assert (forall ((i$ Poly)) (! (=> (has_type i$ INT) (=> (and (<= 0 (%I i$)) (< (%I i$) (vstd!seq.Seq.len.? A&. A& s!))) (<= (%I (%%apply%%0 f! (vstd!seq.Seq.index.? A&. A& s! i$))) high!))) :pattern ((vstd!seq.Seq.index.? A&. A& s! i$)))))
...
(assert (not (=> (= decrease%init0 (vstd!seq.Seq.len.? A&. A& s!)) (or (and (=> (not (= (vstd!seq.Seq.len.? A&. A& s!) 0)) (=> (= tmp%1 (vstd!seq_lib.impl&%0.drop_last.? A&. A& s!)) (and (=> %%location_label%%0 (check_decrease_int (let ((s!$0 tmp%1) (init!$1 init!) (high!$2 high!) (f!$3 f!)) (vstd!seq.Seq.len.? A&. A& s!$0)) decrease%init0 false)) (and (=> %%location_label%%1 (req%lib!verus_extra.seq_lib_v.lemma_seq_fold_left_sum_le. A&. A& tmp%1 init! high! f!)) (=> (ens%lib!verus_extra.seq_lib_v.lemma_seq_fold_left_sum_le. A&. A& tmp%1 init! high! f!) (and (=> %%location_label%%2 true) (=> (<= (Add (Add init! (Mul (Sub (vstd!seq.Seq.len.? A&. A& s!) 1) high!)) high!) (Add init! (Mul (vstd!seq.Seq.len.? A&. A& s!) high!))) %%switch_label%%0))))))) (=> (not (not (= (vstd!seq.Seq.len.? A&. A& s!) 0))) %%switch_label%%0)) (and (not %%switch_label%%0) (=> %%location_label%%3 (<= (%I (vstd!seq_lib.impl&%0.fold_left.? A&. A& $ INT s! (I init!) (Poly%fun%2. (mk_fun (%%lambda%%1 f!))))) (Add init! (Mul (vstd!seq.Seq.len.? A&. A& s!) high!)))))))))
(check-sat)
(exit)
