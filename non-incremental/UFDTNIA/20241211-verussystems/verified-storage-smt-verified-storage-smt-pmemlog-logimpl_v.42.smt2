...
(declare-const spec_ib@ Int)
(declare-const tmp%4 core!result.Result.)
(declare-const bytes@ alloc!vec.Vec<u8./allocator_global%.>.)
...
(assert (is-core!option.Option./Some_ (main!logimpl_v.impl&%0.recover.? (main!pmemspec_t.PersistentMemory.view.? PM&. PM& pm!))))
(assert (%B (main!pmemspec_t.PersistentMemory.inv.? PM&. PM& pm!)))
(assert (> (vstd!seq.Seq.len.? $ (UINT 8) (main!pmemspec_t.PersistentMemory.view.? PM&. PM& pm!)) main!logimpl_v.contents_offset.?))
...
(assert (not (=> (= tmp%2 (I main!logimpl_v.incorruptible_bool_pos.?)) (and (=> %%location_label%%0 (req%main!pmemspec_t.PersistentMemory.read. PM&. PM& pm! tmp%2 (I 8))) (=> (ens%main!pmemspec_t.PersistentMemory.read. PM&. PM& pm! tmp%2 (I 8) tmp%1) (=> (= bytes@ (%Poly%alloc!vec.Vec<u8./allocator_global%.>. tmp%1)) (=> (ens%alloc!vec.impl&%1.as_slice. $ (UINT 8) $ ALLOCATOR_GLOBAL (Poly%alloc!vec.Vec<u8./allocator_global%.>. bytes@) tmp%3) (and (=> %%location_label%%1 (req%vstd!bytes.u64_from_le_bytes. (%Poly%slice%<u8.>. tmp%3))) (=> (ens%vstd!bytes.u64_from_le_bytes. (%Poly%slice%<u8.>. tmp%3) ib@) (=> (= verus_tmp@1 (%Poly%vstd!seq.Seq<int.>. (vstd!seq.Seq.new.? $ INT $ (TYPE%fun%1. $ INT $ INT) (I 8) (Poly%fun%1. (mk_fun (%%lambda%%0 main!logimpl_v.incorruptible_bool_pos.?)))))) (=> (= verus_tmp_addrs@ verus_tmp@1) (=> (= addrs@1 verus_tmp_addrs@) (or (and (=> (or (= ib@ main!pmemspec_t.cdb0_val.?) (= ib@ main!pmemspec_t.cdb1_val.?)) (=> (= tmp%%@ (main!logimpl_v.pm_to_views.? (main!pmemspec_t.PersistentMemory.view.? PM&. PM& pm!))) (=> (= spec_ib@ (%I (tuple%3./tuple%3/0 (%Poly%tuple%3. (Poly%tuple%3. tmp%%@))))) (=> (ens%vstd!bytes.lemma_auto_spec_u64_to_from_le_bytes. 0) (or (and (=> (not (main!pmemspec_t.PersistentMemoryConstants./PersistentMemoryConstants/impervious_to_corruption (%Poly%main!pmemspec_t.PersistentMemoryConstants. (main!pmemspec_t.PersistentMemory.constants.? PM&. PM& pm!)))) (and (=> %%location_label%%2 (req%main!pmemspec_t.axiom_corruption_detecting_boolean. ib@ spec_ib@ addrs@1)) (=> (ens%main!pmemspec_t.axiom_corruption_detecting_boolean. ib@ spec_ib@ addrs@1) %%switch_label%%1))) (=> (not (not (main!pmemspec_t.PersistentMemoryConstants./PersistentMemoryConstants/impervious_to_corruption (%Poly%main!pmemspec_t.PersistentMemoryConstants. (main!pmemspec_t.PersistentMemory.constants.? PM&. PM& pm!))))) %%switch_label%%1)) (and (not %%switch_label%%1) (=> (= tmp%4 (core!result.Result./Ok (I ib@))) %%switch_label%%0))))))) (=> (not (or (= ib@ main!pmemspec_t.cdb0_val.?) (= ib@ main!pmemspec_t.cdb1_val.?))) (=> (= tmp%4 (core!result.Result./Err (Poly%main!main_t.InfiniteLogErr. main!main_t.InfiniteLogErr./CRCMismatch))) %%switch_label%%0))) (and (not %%switch_label%%0) (=> (= result! tmp%4) (=> %%location_label%%3 (ite (is-core!result.Result./Ok_ result!) (let ((ib$ (%I (core!result.Result./Ok/0 (%Poly%core!result.Result. (Poly%core!result.Result. result!)))))) (let ((tmp%%$ (main!logimpl_v.pm_to_views.? (main!pmemspec_t.PersistentMemory.view.? PM&. PM& pm!)))) (let ((spec_ib$ (%I (tuple%3./tuple%3/0 (%Poly%tuple%3. (Poly%tuple%3. tmp%%$)))))) (= ib$ spec_ib$)))) (and (and (is-core!result.Result./Err_ result!) (is-main!main_t.InfiniteLogErr./CRCMismatch_ (%Poly%main!main_t.InfiniteLogErr. (core!result.Result./Err/0 (%Poly%core!result.Result. (Poly%core!result.Result. result!)))))) (not (main!pmemspec_t.PersistentMemoryConstants./PersistentMemoryConstants/impervious_to_corruption (%Poly%main!pmemspec_t.PersistentMemoryConstants. (main!pmemspec_t.PersistentMemory.constants.? PM&. PM& pm!))))))))))))))))))))))
(check-sat)
(exit)
