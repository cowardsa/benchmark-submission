...
(assert (not (and (=> %%location_label%%0 (req%lib!linked_list.impl&%2.is_empty_iff_null. self!)) (=> (ens%lib!linked_list.impl&%2.is_empty_iff_null. self!) (=> (= tmp%2 (lib!linked_list.LL./LL/first (%Poly%lib!linked_list.LL. (Poly%lib!linked_list.LL. self!)))) (=> (ens%core!ptr.mut_ptr.impl&%0.addr. $ TYPE%lib!linked_list.Node. (Poly%ptr_mut%<lib!linked_list.Node.>. tmp%2) tmp%1) (=> (= b! (= tmp%1 0)) (=> %%location_label%%1 (= b! (= (lib!linked_list.impl&%2.len.? (Poly%lib!linked_list.LL. self!)) 0))))))))))
(check-sat)
(exit)
