...
(declare-const cm! lib!commit_mask.CommitMask.)
(declare-const no%param Int)
(declare-const tmp%1 Bool)
(declare-const res@ lib!commit_mask.CommitMask.)
(assert fuel_defaults)
(declare-fun %%array%%0 (Poly Poly Poly Poly Poly Poly Poly Poly) %%Function%%)
(assert (forall ((%%hole%%0 Poly) (%%hole%%1 Poly) (%%hole%%2 Poly) (%%hole%%3 Poly) (%%hole%%4 Poly) (%%hole%%5 Poly) (%%hole%%6 Poly) (%%hole%%7 Poly)) (! (let ((%%x%% (%%array%%0 %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3 %%hole%%4 %%hole%%5 %%hole%%6 %%hole%%7))) (and (= (%%apply%%1 %%x%% 0) %%hole%%0) (= (%%apply%%1 %%x%% 1) %%hole%%1) (= (%%apply%%1 %%x%% 2) %%hole%%2) (= (%%apply%%1 %%x%% 3) %%hole%%3) (= (%%apply%%1 %%x%% 4) %%hole%%4) (= (%%apply%%1 %%x%% 5) %%hole%%5) (= (%%apply%%1 %%x%% 6) %%hole%%6) (= (%%apply%%1 %%x%% 7) %%hole%%7))) :pattern ((%%array%%0 %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3 %%hole%%4 %%hole%%5 %%hole%%6 %%hole%%7)))))
(set-info :comment ";; assertion failed")
(declare-const %%location_label%%0 Bool)
(set-info :comment ";; postcondition not satisfied")
(declare-const %%location_label%%1 Bool)
(assert (not (=> (= res@ (lib!commit_mask.CommitMask./CommitMask (%Poly%array%. (array_new $ (UINT SZ) 8 (%%array%%0 (I 0) (I 0) (I 0) (I 0) (I 0) (I 0) (I 0) (I 0)))))) (=> (ens%lib!commit_mask.lemma_is_bit_set. 0) (=> (ens%lib!commit_mask.impl&%0.lemma_view. res@) (=> (= tmp%1 (ext_eq false (TYPE%vstd!set.Set. $ INT) (Poly%vstd!set.Set<int.>. (lib!commit_mask.impl&%0.view.? (Poly%lib!commit_mask.CommitMask. res@))) (vstd!set.impl&%0.empty.? $ INT))) (and (=> %%location_label%%0 tmp%1) (=> tmp%1 (=> (= cm! res@) (=> %%location_label%%1 (= (lib!commit_mask.impl&%0.view.? (Poly%lib!commit_mask.CommitMask. cm!)) (%Poly%vstd!set.Set<int.>. (vstd!set.impl&%0.empty.? $ INT)))))))))))))
(check-sat)
(exit)
