...
(assert (lib!layout.is_block_ptr.? (Poly%ptr_mut%<u8.>. block_ptr!) (Poly%lib!tokens.BlockId. block_id!)))
(assert (lib!layout.is_segment_ptr.? (Poly%ptr_mut%<lib!types.SegmentHeader.>. segment_ptr!) (Poly%lib!tokens.SegmentId. (lib!tokens.PageId./PageId/segment_id (%Poly%lib!tokens.PageId. (Poly%lib!tokens.PageId. (lib!tokens.BlockId./BlockId/page_id (%Poly%lib!tokens.BlockId. (Poly%lib!tokens.BlockId. block_id!)))))))))
...
(assert (not (=> (ens%core!ptr.mut_ptr.impl&%0.addr. $ (UINT 8) (Poly%ptr_mut%<u8.>. block_ptr!) b@) (=> (ens%core!ptr.mut_ptr.impl&%0.addr. $ TYPE%lib!types.SegmentHeader. (Poly%ptr_mut%<lib!types.SegmentHeader.>. segment_ptr!) s@) (=> (fuel_bool fuel%lib!layout.is_block_ptr1.) (=> (ens%lib!config.const_facts. 0) (=> (= tmp%1 (<= (Sub b@ s@) lib!config.SEGMENT_SIZE.?)) (and (=> %%location_label%%0 tmp%1) (=> tmp%1 (and (=> %%location_label%%1 (uInv SZ (Sub b@ s@))) (and (=> %%location_label%%2 (not (= (uClip SZ lib!config.SLICE_SIZE.?) 0))) (=> (= q@ (uClip SZ (EucDiv (uClip SZ (Sub b@ s@)) (uClip SZ lib!config.SLICE_SIZE.?)))) (and (=> %%location_label%%3 (= lib!config.SLICES_PER_SEGMENT.? (EucDiv lib!config.SEGMENT_SIZE.? lib!config.SLICE_SIZE.?))) (and (=> %%location_label%%4 (<= (Sub b@ s@) lib!config.SEGMENT_SIZE.?)) (and (=> %%location_label%%5 (> lib!config.SLICE_SIZE.? 0)) (=> (<= (EucDiv (Sub b@ s@) lib!config.SLICE_SIZE.?) lib!config.SLICES_PER_SEGMENT.?) (and (=> %%location_label%%6 (uInv SZ (Add s@ lib!config.SIZEOF_SEGMENT_HEADER.?))) (and (=> %%location_label%%7 (uInv SZ (Mul q@ lib!config.SIZEOF_PAGE_HEADER.?))) (and (=> %%location_label%%8 (uInv SZ (Add (uClip SZ (Add s@ lib!config.SIZEOF_SEGMENT_HEADER.?)) (uClip SZ (Mul q@ lib!config.SIZEOF_PAGE_HEADER.?))))) (=> (= h@ (uClip SZ (Add (uClip SZ (Add s@ lib!config.SIZEOF_SEGMENT_HEADER.?)) (uClip SZ (Mul q@ lib!config.SIZEOF_PAGE_HEADER.?))))) (=> (ens%core!ptr.mut_ptr.impl&%0.with_addr. $ (UINT 8) (Poly%ptr_mut%<u8.>. block_ptr!) h@ tmp%2) (=> (ens%vstd!raw_ptr.cast_ptr_to_thin_ptr. $ (UINT 8) $ TYPE%lib!types.Page. tmp%2 tmp%3) (=> (= page_ptr! (%Poly%ptr_mut%<lib!types.Page.>. tmp%3)) (=> %%location_label%%9 (lib!layout.is_page_ptr.? (Poly%ptr_mut%<lib!types.Page.>. page_ptr!) (Poly%lib!tokens.PageId. (lib!tokens.impl&%1.page_id_for_slice.? (Poly%lib!tokens.BlockId. block_id!))))))))))))))))))))))))))))
(check-sat)
(exit)
