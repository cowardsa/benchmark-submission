...
(assert (not (and (=> %%location_label%%0 (req%lib!tokens.heap_u_max_not_in. s!)) (=> (ens%lib!tokens.heap_u_max_not_in. s!) (=> %%location_label%%1 (forall ((id$ Poly)) (! (=> (has_type id$ TYPE%lib!tokens.HeapId.) (=> (vstd!set.impl&%0.contains.? $ TYPE%lib!tokens.HeapId. (Poly%vstd!set.Set<lib!tokens.HeapId.>. s!) id$) (not (= (lib!tokens.HeapId./HeapId/uniq (%Poly%lib!tokens.HeapId. id$)) (lib!tokens.heap_get_unused_uniq_field.? (Poly%vstd!set.Set<lib!tokens.HeapId.>. s!)))))) :pattern ((vstd!set.impl&%0.contains.? $ TYPE%lib!tokens.HeapId. (Poly%vstd!set.Set<lib!tokens.HeapId.>. s!) id$)))))))))
(check-sat)
(exit)
