...
(declare-const tmp%8 main!logimpl_v.PersistentHeaderMetadata.)
(declare-const crc_bytes@ slice%<u8.>.)
(declare-const metadata_bytes@ slice%<u8.>.)
(assert fuel_defaults)
(assert (= (vstd!seq.Seq.len.? $ (UINT 8) (vstd!view.View.view.? $ (SLICE $ (UINT 8)) (Poly%slice%<u8.>. bytes!))) main!logimpl_v.header_size.?))
...
(assert (not (and (=> %%location_label%%0 (uInv 64 (Add main!logimpl_v.header_crc_offset.? 8))) (=> (= tmp%2 (uClip SZ main!logimpl_v.header_crc_offset.?)) (=> (= tmp%3 (uClip SZ (uClip 64 (Add main!logimpl_v.header_crc_offset.? 8)))) (and (=> %%location_label%%1 (req%vstd!slice.slice_subrange. $ (UINT 8) (Poly%slice%<u8.>. bytes!) tmp%2 tmp%3)) (=> (ens%vstd!slice.slice_subrange. $ (UINT 8) (Poly%slice%<u8.>. bytes!) tmp%2 tmp%3 tmp%1) (=> (= crc_bytes@ (%Poly%slice%<u8.>. tmp%1)) (=> (= tmp%5 (uClip SZ main!logimpl_v.header_head_offset.?)) (=> (= tmp%6 (uClip SZ main!logimpl_v.header_size.?)) (and (=> %%location_label%%2 (req%vstd!slice.slice_subrange. $ (UINT 8) (Poly%slice%<u8.>. bytes!) tmp%5 tmp%6)) (=> (ens%vstd!slice.slice_subrange. $ (UINT 8) (Poly%slice%<u8.>. bytes!) tmp%5 tmp%6 tmp%4) (=> (= metadata_bytes@ (%Poly%slice%<u8.>. tmp%4)) (and (=> %%location_label%%3 (req%vstd!bytes.u64_from_le_bytes. crc_bytes@)) (=> (ens%vstd!bytes.u64_from_le_bytes. crc_bytes@ tmp%7) (and (=> %%location_label%%4 (req%main!logimpl_v.bytes_to_metadata. metadata_bytes@)) (=> (ens%main!logimpl_v.bytes_to_metadata. metadata_bytes@ tmp%8) (=> (= out! (main!logimpl_v.PersistentHeader./PersistentHeader (%I (I tmp%7)) (%Poly%main!logimpl_v.PersistentHeaderMetadata. (Poly%main!logimpl_v.PersistentHeaderMetadata. tmp%8)))) (=> %%location_label%%5 (= out! (main!logimpl_v.spec_bytes_to_header.? (vstd!view.View.view.? $ (SLICE $ (UINT 8)) (Poly%slice%<u8.>. bytes!)))))))))))))))))))))))
(check-sat)
(exit)
