...
(set-info :comment ";; page-table/extra.rs:38:1: 38:58 (#0)")
...
(set-info :comment ";; Function-Def lib::extra::leq_add_aligned_less")
(set-info :comment ";; page-table/extra.rs:38:1: 38:58 (#0)")
(declare-const a! Int)
(declare-const b! Int)
(declare-const c! Int)
...
(assert (< 0 b!))
(assert (< a! c!))
(assert (lib!definitions_t.aligned.? (I a!) (I b!)))
(assert (lib!definitions_t.aligned.? (I c!) (I b!)))
...
(assert (not (=> (= tmp%1 (= a! (nClip (Add (nClip (Mul b! (nClip (EucDiv a! b!)))) (nClip (EucMod a! b!)))))) (and (=> %%location_label%%0 tmp%1) (=> tmp%1 (=> (= tmp%2 (= c! (nClip (Add (nClip (Mul b! (nClip (EucDiv c! b!)))) (nClip (EucMod c! b!)))))) (and (=> %%location_label%%1 tmp%2) (=> tmp%2 (=> %%location_label%%2 (<= (nClip (Add a! b!)) c!))))))))))
(check-sat)
(exit)
