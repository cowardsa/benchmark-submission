...
(assert (has_type tmp%1 (TYPE%alloc!vec.Vec. $ (UINT 64) $ ALLOCATOR_GLOBAL)))
(assert (uInv SZ tmp%2))
(assert (has_type tmp%3 (TYPE%core!ops.range.Range. $ (UINT SZ))))
(assert (has_type (Poly%core!ops.range.Range. tmp%4) (TYPE%core!ops.range.Range. $ (UINT SZ))))
