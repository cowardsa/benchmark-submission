...
(assert (%B (verismo!addr_e.addr_interface.AddrTrait.spec_check_valid_addr_requires.? $ (TYPE%verismo!tspec.security.sectype.SecType. $ (UINT SZ) $ TYPE%tuple%0.) $ (TYPE%verismo!tspec.security.sectype.SecType. $ (UINT SZ) $ TYPE%tuple%0.) self! size!)))
...
(assert (not (and (=> %%location_label%%0 (req%verismo!tspec.security.sectype.impl&%77.into. $ TYPE%tuple%0. self!)) (=> (ens%verismo!tspec.security.sectype.impl&%77.into. $ TYPE%tuple%0. self! addr@) (and (=> %%location_label%%1 (req%verismo!tspec.security.sectype.impl&%77.into. $ TYPE%tuple%0. size!)) (=> (ens%verismo!tspec.security.sectype.impl&%77.into. $ TYPE%tuple%0. size! size@) (or (and (=> (<= size@ verismo!arch.addr_s.def_s.VM_MEM_SIZE.?) (and (=> %%location_label%%2 (uInv SZ (Sub verismo!arch.addr_s.def_s.VM_MEM_SIZE.? size@))) (=> (= tmp%1 (<= addr@ (uClip SZ (Sub verismo!arch.addr_s.def_s.VM_MEM_SIZE.? size@)))) %%switch_label%%0))) (=> (not (<= size@ verismo!arch.addr_s.def_s.VM_MEM_SIZE.?)) (=> (= tmp%1 false) %%switch_label%%0))) (and (not %%switch_label%%0) (=> (= %return! (B tmp%1)) (=> %%location_label%%3 (%B (B (= %return! (verismo!addr_e.addr_interface.AddrTrait.spec_valid_addr_with.? $ (TYPE%verismo!tspec.security.sectype.SecType. $ (UINT SZ) $ TYPE%tuple%0.) $ (TYPE%verismo!tspec.security.sectype.SecType. $ (UINT SZ) $ TYPE%tuple%0.) self! (verismo!tspec.cast.VTypeCast.vspec_cast_to.? $ INT $ NAT (verismo!addr_e.addr_interface.AddrTrait.addrt_to_int.? $ (TYPE%verismo!tspec.security.sectype.SecType. $ (UINT SZ) $ TYPE%tuple%0.) $ (TYPE%verismo!tspec.security.sectype.SecType. $ (UINT SZ) $ TYPE%tuple%0.) size!))))))))))))))))
(check-sat)
(exit)
