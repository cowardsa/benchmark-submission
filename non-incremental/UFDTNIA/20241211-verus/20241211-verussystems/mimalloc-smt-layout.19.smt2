...
(assert (forall ((id FuelId)) (! (or (= (fuel_bool id) (fuel_bool_default id)) (= id fuel%lib!layout.is_block_ptr.)) :pattern ((fuel_bool id)))))
...
(assert (not (=> (ens%lib!linked_list.size_of_node. 0) (=> (ens%lib!layout.block_ptr_aligned_to_word. 0) (=> %%location_label%%0 (= (EucMod (vstd!raw_ptr.spec_cast_ptr_to_usize.? $ (UINT 8) (Poly%ptr_mut%<u8.>. ptr!)) 4) 0))))))
(check-sat)
(exit)
