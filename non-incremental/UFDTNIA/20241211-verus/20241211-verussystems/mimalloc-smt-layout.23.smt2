...
(assert (not (and (=> %%location_label%%0 (let ((tmp%%$ (lib!layout.start_offset.? (I block_size!)))) (and (<= 0 tmp%%$) (<= tmp%%$ lib!config.SLICE_SIZE.?)))) (=> %%location_label%%1 (or (= (lib!layout.start_offset.? (I block_size!)) 0) (= (lib!layout.start_offset.? (I block_size!)) (Mul 3 lib!config.MAX_ALIGN_GUARANTEE.?)))))))
(check-sat)
(exit)
