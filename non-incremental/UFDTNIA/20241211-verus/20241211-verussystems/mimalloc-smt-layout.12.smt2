...
(assert (uInv SZ page_start!))
(assert (uInv SZ block_size!))
(assert (uInv SZ idx!))
(assert (has_type (Poly%lib!tokens.PageId. page_id!) TYPE%lib!tokens.PageId.))
(assert (= page_start! (lib!layout.block_start_at.? (Poly%lib!tokens.PageId. page_id!) (I block_size!) (I 0))))
(assert (<= (Add (lib!layout.block_start_at.? (Poly%lib!tokens.PageId. page_id!) (I block_size!) (I 0)) (Mul idx! block_size!)) (Add (lib!layout.segment_start.? (Poly%lib!tokens.SegmentId. (lib!tokens.PageId./PageId/segment_id (%Poly%lib!tokens.PageId. (Poly%lib!tokens.PageId. page_id!))))) lib!config.SEGMENT_SIZE.?)))
(assert (< (Add (lib!layout.segment_start.? (Poly%lib!tokens.SegmentId. (lib!tokens.PageId./PageId/segment_id (%Poly%lib!tokens.PageId. (Poly%lib!tokens.PageId. page_id!))))) lib!config.SEGMENT_SIZE.?) (- (uHi SZ) 1)))
...
(assert (not (=> (ens%lib!config.const_facts. 0) (and (=> %%location_label%%0 (>= block_size! 0)) (and (=> %%location_label%%1 (>= idx! 0)) (=> (>= (Mul block_size! idx!) 0) (and (=> %%location_label%%2 true) (=> (= (Mul block_size! idx!) (Mul idx! block_size!)) (and (=> %%location_label%%3 (uInv SZ (Mul block_size! idx!))) (and (=> %%location_label%%4 (uInv SZ (Add page_start! (uClip SZ (Mul block_size! idx!))))) (=> (= p@ (uClip SZ (Add page_start! (uClip SZ (Mul block_size! idx!))))) (=> (= p! p@) (and (=> %%location_label%%5 (= p! (lib!layout.block_start_at.? (Poly%lib!tokens.PageId. page_id!) (I block_size!) (I idx!)))) (=> %%location_label%%6 (= p! (Add page_start! (Mul idx! block_size!)))))))))))))))))
(check-sat)
(exit)
