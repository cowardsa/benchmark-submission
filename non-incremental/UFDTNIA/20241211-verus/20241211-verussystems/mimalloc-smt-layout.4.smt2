...
(assert (not (=> (fuel_bool fuel%lib!layout.is_block_ptr1.) (=> %%location_label%%0 (forall ((p$ Poly) (block_id$ Poly)) (! (=> (and (has_type p$ (PTR $ (UINT 8))) (has_type block_id$ TYPE%lib!tokens.BlockId.)) (=> (lib!layout.is_block_ptr.? p$ block_id$) (>= (lib!tokens.BlockId./BlockId/block_size (%Poly%lib!tokens.BlockId. block_id$)) (vstd!layout.size_of.? $ TYPE%lib!linked_list.Node.)))) :pattern ((lib!layout.is_block_ptr.? p$ block_id$))))))))
(check-sat)
(exit)
