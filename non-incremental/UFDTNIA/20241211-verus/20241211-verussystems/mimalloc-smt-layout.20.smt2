...
(assert (not (=> (ens%lib!config.const_facts. 0) (=> (= tmp%1 (= lib!config.COMMIT_SIZE.? 65536)) (and (=> %%location_label%%0 tmp%1) (=> tmp%1 (=> %%location_label%%1 (= (EucMod (lib!layout.segment_start.? (Poly%lib!tokens.SegmentId. segment_id!)) lib!config.COMMIT_SIZE.?) 0))))))))
(check-sat)
(exit)
