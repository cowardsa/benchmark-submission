...
(set-info :comment ";; page-table/extra.rs:19:1: 19:68 (#0)")
...
(set-info :comment ";; Function-Def lib::extra::mod_mult_zero_implies_mod_zero")
(set-info :comment ";; page-table/extra.rs:19:1: 19:68 (#0)")
(declare-const a! Int)
(declare-const b! Int)
(declare-const c! Int)
...
(assert (<= 0 b!))
(assert (<= 0 c!))
(assert (lib!definitions_t.aligned.? (I a!) (I (nClip (Mul b! c!)))))
(assert (> b! 0))
(assert (> c! 0))
...
(assert (not (=> (fuel_bool fuel%vstd!arithmetic.div_mod.lemma_mod_mod.) (=> (fuel_bool fuel%vstd!arithmetic.div_mod.lemma_mod_breakdown.) (=> (= tmp%1 (= (nClip (EucMod (nClip (EucMod a! (nClip (Mul b! c!)))) b!)) 0)) (and (=> %%location_label%%0 tmp%1) (=> tmp%1 (=> %%location_label%%1 (lib!definitions_t.aligned.? (I a!) (I b!))))))))))
(check-sat)
(exit)
