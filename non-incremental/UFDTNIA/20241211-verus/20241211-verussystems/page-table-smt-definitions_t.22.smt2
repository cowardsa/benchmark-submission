...
(assert (not (=> (= decrease%init0 (%I i!)) (or (and (=> (= (%I i!) 0) (=> (= tmp%3 (vstd!seq.Seq.empty.? T&. T&)) %%switch_label%%0)) (=> (not (= (%I i!) 0)) (=> (= tmp%2 (I (nClip (Sub (%I i!) 1)))) (and (=> %%location_label%%0 (check_decrease_int (let ((i!$0 (%I tmp%2)) (e!$1 e!)) i!$0) decrease%init0 false)) (=> (has_type tmp%1 (TYPE%vstd!seq.Seq. T&. T&)) (=> (= tmp%1 (lib!definitions_t.new_seq.? T&. T& (I (nClip (Sub (%I i!) 1))) e!)) (=> (= tmp%3 (vstd!seq.Seq.push.? T&. T& tmp%1 e!)) %%switch_label%%0))))))) (not %%switch_label%%0)))))
(check-sat)
(exit)
