...
(assert (%B (verismo!addr_e.addr_interface.PageTrait.spec_check_valid_pn_requires.? $ (UINT SZ) $ (UINT SZ) self! size!)))
(declare-const %%switch_label%%0 Bool)
(set-info :comment ";; possible arithmetic underflow/overflow")
(declare-const %%location_label%%0 Bool)
(set-info :comment ";; postcondition not satisfied")
(declare-const %%location_label%%1 Bool)
(assert (not (=> (= addr@ (uClip SZ (%I self!))) (=> (= size@ (uClip SZ (%I size!))) (or (and (=> (<= size@ verismo!arch.addr_s.def_s.VM_PAGE_NUM.?) (and (=> %%location_label%%0 (uInv SZ (Sub verismo!arch.addr_s.def_s.VM_PAGE_NUM.? size@))) (=> (= tmp%1 (<= addr@ (uClip SZ (Sub verismo!arch.addr_s.def_s.VM_PAGE_NUM.? size@)))) %%switch_label%%0))) (=> (not (<= size@ verismo!arch.addr_s.def_s.VM_PAGE_NUM.?)) (=> (= tmp%1 false) %%switch_label%%0))) (and (not %%switch_label%%0) (=> (= %return! (B tmp%1)) (=> %%location_label%%1 (%B (B (= %return! (verismo!addr_e.addr_interface.PageTrait.spec_valid_pn_with.? $ (UINT SZ) $ (UINT SZ) self! (verismo!tspec.cast.VTypeCast.vspec_cast_to.? $ INT $ NAT (verismo!addr_e.addr_interface.PageTrait.paget_to_int.? $ (UINT SZ) $ (UINT SZ) size!))))))))))))))
(check-sat)
(exit)
