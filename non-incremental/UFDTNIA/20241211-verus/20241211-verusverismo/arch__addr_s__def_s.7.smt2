...
(declare-const self! verismo!arch.addr_s.def_s.PageSize.)
(assert fuel_defaults)
(set-info :comment ";; postcondition not satisfied")
(declare-const %%location_label%%0 Bool)
(assert (not (=> (= ret! (let ((ret$ (ite (is-verismo!arch.addr_s.def_s.PageSize./Size4k_ self!) 0 1))) ret$)) (=> %%location_label%%0 (= ret! (%I (verismo!tspec.integer.IntValue.as_int.? $ TYPE%verismo!arch.addr_s.def_s.PageSize. (Poly%verismo!arch.addr_s.def_s.PageSize. self!))))))))
(check-sat)
(exit)
