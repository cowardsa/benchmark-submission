...
(assert (%B (verismo!addr_e.addr_interface.AddrTrait.spec_valid_addr_with.? $ (UINT SZ) $ (UINT SZ) self! (I 0))))
...
(assert (not (and (=> %%location_label%%0 (not (= (uClip SZ verismo!arch.addr_s.def_s.PAGE_SIZE.?) 0))) (=> (= %return! (I (uClip SZ (EucDiv (%I self!) (uClip SZ verismo!arch.addr_s.def_s.PAGE_SIZE.?))))) (and (=> %%location_label%%1 (= %return! (verismo!addr_e.addr_interface.AddrTrait.spec_to_page.? $ (UINT SZ) $ (UINT SZ) self!))) (=> %%location_label%%2 (%B (verismo!addr_e.addr_interface.AddrTrait.spec_ensures_to_page.? $ (UINT SZ) $ (UINT SZ) self! %return!))))))))
(check-sat)
(exit)
