...
(assert (%B (verismo!addr_e.addr_interface.AddrTrait.spec_check_valid_addr_requires.? $ (UINT 64) $ (UINT 64) self! size!)))
(declare-const %%switch_label%%0 Bool)
(set-info :comment ";; possible arithmetic underflow/overflow")
(declare-const %%location_label%%0 Bool)
(set-info :comment ";; postcondition not satisfied")
(declare-const %%location_label%%1 Bool)
(assert (not (=> (= addr@ (uClip SZ (%I self!))) (=> (= size@ (uClip SZ (%I size!))) (or (and (=> (<= size@ verismo!arch.addr_s.def_s.VM_MEM_SIZE.?) (and (=> %%location_label%%0 (uInv SZ (Sub verismo!arch.addr_s.def_s.VM_MEM_SIZE.? size@))) (=> (= tmp%1 (<= addr@ (uClip SZ (Sub verismo!arch.addr_s.def_s.VM_MEM_SIZE.? size@)))) %%switch_label%%0))) (=> (not (<= size@ verismo!arch.addr_s.def_s.VM_MEM_SIZE.?)) (=> (= tmp%1 false) %%switch_label%%0))) (and (not %%switch_label%%0) (=> (= %return! (B tmp%1)) (=> %%location_label%%1 (%B (B (= %return! (verismo!addr_e.addr_interface.AddrTrait.spec_valid_addr_with.? $ (UINT 64) $ (UINT 64) self! (verismo!tspec.cast.VTypeCast.vspec_cast_to.? $ INT $ NAT (verismo!addr_e.addr_interface.AddrTrait.addrt_to_int.? $ (UINT 64) $ (UINT 64) size!))))))))))))))
(check-sat)
(exit)
