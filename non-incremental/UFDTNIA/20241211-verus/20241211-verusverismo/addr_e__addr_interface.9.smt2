...
(assert (%B (verismo!addr_e.addr_interface.PageTrait.spec_valid_pn_with.? $ (UINT SZ) $ (UINT SZ) self! (I 0))))
...
(assert (not (and (=> %%location_label%%0 (uInv SZ (Mul (%I self!) 4096))) (=> (= %return! (I (uClip SZ (Mul (%I self!) 4096)))) (and (=> %%location_label%%1 (= %return! (verismo!addr_e.addr_interface.PageTrait.spec_to_addr.? $ (UINT SZ) $ (UINT SZ) self!))) (=> %%location_label%%2 (%B (verismo!addr_e.addr_interface.PageTrait.spec_ensures_to_addr.? $ (UINT SZ) $ (UINT SZ) self! %return!))))))))
(check-sat)
(exit)
