...
;; defqtvc
 ;; File "x86.ads", line 38, characters 0-0
  ...
  (=>
  (and (= (readmem8 o memory2) ((_ extract 7 0) (bvand val__ #x00FF)))
  (forall ((x (_ BitVec 64)))
  (=> (and (bvule #x0000000000000000 x) (bvule x #xFFFFFFFFFFFFFFFF))
  (or (= x o) (= (to_rep (select memory2 x)) (to_rep (select memory1 x)))))))
  (forall ((o1 (_ BitVec 16)))
  (=> (ite (< 8 16) (= o1 (bvlshr val__ ((_ int_to_bv 16) 8))) (= o1 #x0000))
  (let ((o2 (bvadd addr #x0000000000000001)))
  (forall ((memory3 (Array (_ BitVec 64) unsigned8)))
  (=> (= memory3 memory2)
  (forall ((memory4 (Array (_ BitVec 64) unsigned8)))
  (=>
  (forall ((o3 (_ BitVec 64)) (memory5 (Array (_ BitVec 64) unsigned8)))
  (readmem8__function_guard (readmem8 o3 memory5) o3 memory5))
  (=>
  (and (= (readmem8 o2 memory4) ((_ extract 7 0) (bvand o1 #x00FF)))
  (forall ((x (_ BitVec 64)))
  (=> (and (bvule #x0000000000000000 x) (bvule x #xFFFFFFFFFFFFFFFF))
  (or (= x o2) (= (to_rep (select memory4 x)) (to_rep (select memory3 x)))))))
  (=>
  (forall ((memory5 (Array (_ BitVec 64) unsigned8)))
  (readmem8__function_guard (readmem8 addr memory5) addr memory5))
  (=>
  (forall ((memory5 (Array (_ BitVec 64) unsigned8)))
  (readmem8__function_guard
  (readmem8 (bvadd addr #x0000000000000001) memory5)
  (bvadd addr #x0000000000000001) memory5))
  (= (bvor ((_ zero_extend 8) (readmem8 addr memory4)) (ite (< 8 16)
                                                       (bvshl ((_ zero_extend 8) 
                                                       (readmem8
                                                       (bvadd addr #x0000000000000001)
                                                       memory4)) ((_ int_to_bv 16) 8))
                                                       #x0000)) val__))))))))))))))))))))))
(check-sat)
(exit)
