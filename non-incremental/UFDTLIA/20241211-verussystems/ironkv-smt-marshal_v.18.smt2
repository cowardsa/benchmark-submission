...
(assert (not (or (and (=> (is-core!option.Option./None_ (%Poly%core!option.Option. self!)) (=> (= tmp%4 true) %%switch_label%%0)) (=> (not (is-core!option.Option./None_ (%Poly%core!option.Option. self!))) (=> (= x@ (core!option.Option./Some/0 (%Poly%core!option.Option. self!))) (=> (ens%lib!marshal_v.Marshalable._is_marshalable. T&. T& x@ tmp%1) (or (and (=> (%B tmp%1) (and (=> %%location_label%%0 (req%lib!marshal_v.Marshalable.serialized_size. T&. T& x@)) (=> (ens%lib!marshal_v.Marshalable.serialized_size. T&. T& x@ tmp%2) (and (=> %%location_label%%1 (uInv SZ (Sub (- (uHi SZ) 1) 1))) (=> (= tmp%3 (<= (%I tmp%2) (uClip SZ (Sub (- (uHi SZ) 1) 1)))) %%switch_label%%1))))) (=> (not (%B tmp%1)) (=> (= tmp%3 false) %%switch_label%%1))) (and (not %%switch_label%%1) (=> (= tmp%4 tmp%3) %%switch_label%%0))))))) (and (not %%switch_label%%0) (=> (= %return! (B tmp%4)) (=> %%location_label%%2 (= %return! (lib!marshal_v.Marshalable.is_marshalable.? $ (TYPE%core!option.Option. T&. T&) self!))))))))
(check-sat)
(exit)
