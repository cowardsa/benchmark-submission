...
(assert (not (=> (= tmp%2 (Poly%alloc!vec.Vec<u8./allocator_global%.>. (lib!io_t.EndPoint./EndPoint/id (%Poly%lib!io_t.EndPoint. self!)))) (=> (ens%lib!marshal_v.Marshalable._is_marshalable. $ (TYPE%alloc!vec.Vec. $ (UINT 8) $ ALLOCATOR_GLOBAL) tmp%2 tmp%1) (=> (= %return! tmp%1) (=> %%location_label%%0 (= %return! (lib!marshal_v.Marshalable.is_marshalable.? $ TYPE%lib!io_t.EndPoint. self!))))))))
(check-sat)
(exit)
