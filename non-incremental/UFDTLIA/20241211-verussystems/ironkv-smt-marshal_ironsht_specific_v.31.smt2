...
(assert (not (=> (= tmp%2 (Poly%alloc!vec.Vec<u8./allocator_global%.>. (lib!io_t.EndPoint./EndPoint/id (%Poly%lib!io_t.EndPoint. self!)))) (and (=> %%location_label%%0 (req%lib!marshal_v.Marshalable.serialized_size. $ (TYPE%alloc!vec.Vec. $ (UINT 8) $ ALLOCATOR_GLOBAL) tmp%2)) (=> (ens%lib!marshal_v.Marshalable.serialized_size. $ (TYPE%alloc!vec.Vec. $ (UINT 8) $ ALLOCATOR_GLOBAL) tmp%2 tmp%1) (=> (= %return! tmp%1) (=> %%location_label%%1 (= (%I %return!) (vstd!seq.Seq.len.? $ (UINT 8) (lib!marshal_v.Marshalable.ghost_serialize.? $ TYPE%lib!io_t.EndPoint. self!))))))))))
(check-sat)
(exit)
