...
(assert (not (=> (ens%alloc!vec.impl&%1.len. $ (UINT 8) $ ALLOCATOR_GLOBAL self! len@) (or (and (=> (<= len@ (- (uHi SZ) 1)) (=> (= tmp%1 (%B (lib!marshal_v.Marshalable.is_marshalable.? $ (UINT SZ) (I len@)))) (and (=> %%location_label%%0 tmp%1) (=> tmp%1 %%switch_label%%1)))) (=> (not (<= len@ (- (uHi SZ) 1))) %%switch_label%%1)) (and (not %%switch_label%%1) (or (and (=> (<= len@ (- (uHi SZ) 1)) (and (=> %%location_label%%1 (req%lib!marshal_v.Marshalable.serialized_size. $ (UINT SZ) (I len@))) (=> (ens%lib!marshal_v.Marshalable.serialized_size. $ (UINT SZ) (I len@) tmp%2) (and (=> %%location_label%%2 (uInv SZ (Sub (- (uHi SZ) 1) len@))) (=> (= tmp%3 (<= (%I tmp%2) (uClip SZ (Sub (- (uHi SZ) 1) len@)))) %%switch_label%%0))))) (=> (not (<= len@ (- (uHi SZ) 1))) (=> (= tmp%3 false) %%switch_label%%0))) (and (not %%switch_label%%0) (=> (= %return! (B tmp%3)) (=> %%location_label%%3 (= %return! (lib!marshal_v.Marshalable.is_marshalable.? $ (TYPE%alloc!vec.Vec. $ (UINT 8) $ ALLOCATOR_GLOBAL) self!)))))))))))
(check-sat)
(exit)
