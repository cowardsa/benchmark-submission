...
(declare-const self! lib!io_t.EndPoint.)
(declare-const tmp%1 Int)
(declare-const tmp%2 alloc!vec.Vec<u8./allocator_global%.>.)
(assert fuel_defaults)
(set-info :comment ";; postcondition not satisfied")
(declare-const %%location_label%%0 Bool)
(assert (not (=> (= tmp%2 (lib!io_t.EndPoint./EndPoint/id (%Poly%lib!io_t.EndPoint. (Poly%lib!io_t.EndPoint. self!)))) (=> (ens%alloc!vec.impl&%1.len. $ (UINT 8) $ ALLOCATOR_GLOBAL (Poly%alloc!vec.Vec<u8./allocator_global%.>. tmp%2) tmp%1) (=> (= out! (< tmp%1 1048576)) (=> %%location_label%%0 (= out! (lib!abstract_end_point_t.impl&%0.valid_physical_address.? (Poly%lib!abstract_end_point_t.AbstractEndPoint. (lib!io_t.impl&%4.view.? (Poly%lib!io_t.EndPoint. self!)))))))))))
(check-sat)
(exit)
