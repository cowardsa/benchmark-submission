...
(assert (vstd!set.impl&%0.finite.? $ NAT (Poly%vstd!set.Set<nat.>. s!)))
...
(assert (not (=> (= decrease%init0 (vstd!set.impl&%0.len.? $ NAT (Poly%vstd!set.Set<nat.>. s!))) (or (and (=> (vstd!set_lib.impl&%0.is_empty.? $ NAT (Poly%vstd!set.Set<nat.>. s!)) (=> (= tmp%3 0) %%switch_label%%0)) (=> (not (vstd!set_lib.impl&%0.is_empty.? $ NAT (Poly%vstd!set.Set<nat.>. s!))) (=> (= v1@ (%I (vstd!set.impl&%0.choose.? $ NAT (Poly%vstd!set.Set<nat.>. s!)))) (=> (= tmp%1 (%Poly%vstd!set.Set<nat.>. (vstd!set.impl&%0.remove.? $ NAT (Poly%vstd!set.Set<nat.>. s!) (I v1@)))) (and (=> %%location_label%%0 (check_decrease_int (let ((s!$0 tmp%1)) (vstd!set.impl&%0.len.? $ NAT (Poly%vstd!set.Set<nat.>. s!$0))) decrease%init0 false)) (and (=> %%location_label%%1 (req%lib!spec.unbounded_log.max_of_set. tmp%1)) (=> (ens%lib!spec.unbounded_log.max_of_set. tmp%1 v2@) (=> (= tmp%2 (forall ((x$ Poly)) (! (=> (has_type x$ NAT) (=> (and (vstd!set.impl&%0.contains.? $ NAT (Poly%vstd!set.Set<nat.>. s!) x$) (not (= (%I x$) v1@))) (vstd!set.impl&%0.contains.? $ NAT (vstd!set.impl&%0.remove.? $ NAT (Poly%vstd!set.Set<nat.>. s!) (I v1@)) x$))) :pattern ((vstd!set.impl&%0.contains.? $ NAT (Poly%vstd!set.Set<nat.>. s!) x$)) ))) (and (=> %%location_label%%2 tmp%2) (=> tmp%2 (=> (= tmp%3 (ite (>= v1@ v2@) v1@ v2@)) %%switch_label%%0))))))))))) (and (not %%switch_label%%0) (=> (= r! tmp%3) (=> %%location_label%%3 (forall ((x$ Poly)) (! (=> (has_type x$ NAT) (=> (vstd!set.impl&%0.contains.? $ NAT (Poly%vstd!set.Set<nat.>. s!) x$) (<= (%I x$) r!))) :pattern ((vstd!set.impl&%0.contains.? $ NAT (Poly%vstd!set.Set<nat.>. s!) x$)) )))))))))
(check-sat)
(exit)
