...
(assert (not (=> (ens%lib!hashmap_t.impl&%0.to_vec. (%Poly%lib!hashmap_t.CKeyHashMap. self!) tmp%1) (and (=> %%location_label%%0 (req%lib!marshal_v.Marshalable.serialized_size. $ (TYPE%alloc!vec.Vec. $ TYPE%lib!hashmap_t.CKeyKV. $ ALLOCATOR_GLOBAL) (Poly%alloc!vec.Vec<lib!hashmap_t.CKeyKV./allocator_global%.>. tmp%1))) (=> (ens%lib!marshal_v.Marshalable.serialized_size. $ (TYPE%alloc!vec.Vec. $ TYPE%lib!hashmap_t.CKeyKV. $ ALLOCATOR_GLOBAL) (Poly%alloc!vec.Vec<lib!hashmap_t.CKeyKV./allocator_global%.>. tmp%1) tmp%2) (=> (= %return! tmp%2) (=> %%location_label%%1 (= (%I %return!) (vstd!seq.Seq.len.? $ (UINT 8) (lib!marshal_v.Marshalable.ghost_serialize.? $ TYPE%lib!hashmap_t.CKeyHashMap. self!))))))))))
(check-sat)
(exit)
