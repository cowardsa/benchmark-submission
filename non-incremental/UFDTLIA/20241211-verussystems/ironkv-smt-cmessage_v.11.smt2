...
(declare-const val! alloc!vec.Vec<u8./allocator_global%.>.)
(declare-const tmp%1 Int)
(assert fuel_defaults)
(set-info :comment ";; postcondition not satisfied")
(declare-const %%location_label%%0 Bool)
(assert (not (=> (ens%alloc!vec.impl&%1.len. $ (UINT 8) $ ALLOCATOR_GLOBAL (Poly%alloc!vec.Vec<u8./allocator_global%.>. val!) tmp%1) (=> (= b! (< tmp%1 1024)) (=> %%location_label%%0 (= b! (lib!app_interface_t.valid_value.? (vstd!view.View.view.? $ (TYPE%alloc!vec.Vec. $ (UINT 8) $ ALLOCATOR_GLOBAL) (Poly%alloc!vec.Vec<u8./allocator_global%.>. val!)))))))))
(check-sat)
(exit)
