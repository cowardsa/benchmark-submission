...
(declare-const down_keys! Poly)
(declare-const k@ Poly)
(declare-const k$1@ Poly)
(declare-const kept_buffer@ lib!betree.Buffer_v.SimpleBuffer.)
(declare-const moved_buffer@ lib!betree.Buffer_v.SimpleBuffer.)
(declare-const out_children_map@ vstd!map.Map<lib!spec.KeyType_t.Key./lib!betree.PagedBetree_v.BetreeNode.>.)
(assert fuel_defaults)
(assert (has_type self! TYPE%lib!betree.PagedBetree_v.BetreeNode.))
(assert (has_type down_keys! (TYPE%vstd!set.Set. $ TYPE%lib!spec.KeyType_t.Key.)))
