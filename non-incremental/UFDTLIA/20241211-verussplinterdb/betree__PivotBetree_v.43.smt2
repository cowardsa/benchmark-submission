...
(assert (forall ((%%hole%%0 Poly) (k$ Poly)) (! (= (%%apply%%0 (%%lambda%%6 %%hole%%0) k$) %%hole%%0) :pattern ((%%apply%%0 (%%lambda%%6 %%hole%%0) k$)))))
(assert (=> (fuel_bool fuel%lib!betree.Buffer_v.all_keys.) (forall ((no%param Poly)) (! (= (lib!betree.Buffer_v.all_keys.? no%param) (%Poly%vstd!set.Set<lib!spec.KeyType_t.Key.>. (vstd!set.impl&%0.new.? $ TYPE%lib!spec.KeyType_t.Key. $ (TYPE%fun%1. $ TYPE%lib!spec.KeyType_t.Key. $ BOOL) (Poly%fun%1. (mk_fun (%%lambda%%6 (B true))))))) :pattern ((lib!betree.Buffer_v.all_keys.? no%param)) ))))
...
(assert (forall ((self! Poly) (child_idx! Poly)) (! (= (req%lib!betree.PivotBetree_v.impl&%1.flush. self! child_idx!) (=> %%global_location_label%%56 (lib!betree.PivotBetree_v.impl&%1.can_flush.? self! child_idx!))) :pattern ((req%lib!betree.PivotBetree_v.impl&%1.flush. self! child_idx!)) )))
...
(set-info :comment ";; Function-Recommends lib::betree::PivotBetree_v::empty_image")
(set-info :comment ";; betree/PivotBetree_v.rs:24:1: 24:57 (#0)")
