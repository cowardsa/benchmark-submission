...
(assert (has_type x! (UINT 64)))
(set-info :comment ";; postcondition not satisfied")
(declare-const %%location_label%%0 Bool)
(set-info :comment ";; postcondition not satisfied")
(declare-const %%location_label%%1 Bool)
(assert (not (=> (ens%vstd!bytes.u64_to_le_bytes. (%I x!) tmp%1) (=> (= %return! (Poly%alloc!vec.Vec<u8./allocator_global%.>. tmp%1)) (and (=> %%location_label%%0 (= (vstd!view.View.view.? $ (TYPE%alloc!vec.Vec. $ (UINT 8) $ ALLOCATOR_GLOBAL) %return!) (lib!marshalling.IntegerMarshalling_v.IntFormattable.spec_to_le_bytes.? $ (UINT 64) x!))) (=> %%location_label%%1 (= (vstd!seq.Seq.len.? $ (UINT 8) (vstd!view.View.view.? $ (TYPE%alloc!vec.Vec. $ (UINT 8) $ ALLOCATOR_GLOBAL) %return!)) (%I (lib!marshalling.StaticallySized_v.StaticallySized.uniform_size.? $ (UINT 64))))))))))
(check-sat)
(exit)
