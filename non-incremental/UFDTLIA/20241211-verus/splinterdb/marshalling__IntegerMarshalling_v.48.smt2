...
(assert (has_type s! (SLICE $ (UINT 8))))
(assert (= (vstd!seq.Seq.len.? $ (UINT 8) (vstd!view.View.view.? $ (SLICE $ (UINT 8)) s!)) (%I (lib!marshalling.StaticallySized_v.StaticallySized.uniform_size.? $ (UINT 64)))))
(set-info :comment ";; precondition not satisfied")
(declare-const %%location_label%%0 Bool)
(set-info :comment ";; postcondition not satisfied")
(declare-const %%location_label%%1 Bool)
(assert (not (and (=> %%location_label%%0 (req%vstd!bytes.u64_from_le_bytes. (%Poly%slice%<u8.>. s!))) (=> (ens%vstd!bytes.u64_from_le_bytes. (%Poly%slice%<u8.>. s!) tmp%1) (=> (= %return! (I tmp%1)) (=> %%location_label%%1 (= %return! (lib!marshalling.IntegerMarshalling_v.IntFormattable.spec_from_le_bytes.? $ (UINT 64) (vstd!view.View.view.? $ (SLICE $ (UINT 8)) s!)))))))))
(check-sat)
(exit)
