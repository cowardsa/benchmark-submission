...
(assert (forall ((i$ Poly)) (! (=> (has_type i$ INT) (=> (and (<= 0 (%I i$)) (< (%I i$) (vstd!seq.Seq.len.? $ (TYPE%vstd!set.Set. A&. A&) sets!))) (vstd!set.impl&%0.finite.? A&. A& (vstd!seq.Seq.index.? $ (TYPE%vstd!set.Set. A&. A&) sets! i$)))) :pattern ((vstd!seq.Seq.index.? $ (TYPE%vstd!set.Set. A&. A&) sets! i$)) )))
...
(assert (not (=> (= decrease%init0 (vstd!seq.Seq.len.? $ (TYPE%vstd!set.Set. A&. A&) sets!)) (or (and (=> (> (vstd!seq.Seq.len.? $ (TYPE%vstd!set.Set. A&. A&) sets!) 0) (=> (= tmp%1 (vstd!seq_lib.impl&%0.drop_last.? $ (TYPE%vstd!set.Set. A&. A&) sets!)) (and (=> %%location_label%%0 (check_decrease_int (let ((sets!$0 tmp%1)) (vstd!seq.Seq.len.? $ (TYPE%vstd!set.Set. A&. A&) sets!$0)) decrease%init0 false)) (and (=> %%location_label%%1 (req%lib!betree.Utils_v.lemma_union_seq_of_sets_finite. A&. A& tmp%1)) (=> (ens%lib!betree.Utils_v.lemma_union_seq_of_sets_finite. A&. A& tmp%1) %%switch_label%%0))))) (=> (not (> (vstd!seq.Seq.len.? $ (TYPE%vstd!set.Set. A&. A&) sets!) 0)) %%switch_label%%0)) (and (not %%switch_label%%0) (=> %%location_label%%2 (vstd!set.impl&%0.finite.? A&. A& (lib!betree.Utils_v.union_seq_of_sets.? A&. A& sets!))))))))
(check-sat)
(exit)
