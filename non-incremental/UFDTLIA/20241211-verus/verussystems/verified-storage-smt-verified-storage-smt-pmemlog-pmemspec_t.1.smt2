...
(declare-const %return! vstd!seq.Seq<u8.>.)
(declare-const prewrite_contents! Poly)
(declare-const write_addr! Poly)
(declare-const write_bytes! Poly)
(declare-const addr@ Poly)
(assert fuel_defaults)
(assert (has_type prewrite_contents! (TYPE%vstd!seq.Seq. $ (UINT 8))))
(assert (has_type write_addr! INT))
(assert (has_type write_bytes! (TYPE%vstd!seq.Seq. $ (UINT 8))))
(assert (not true))
(check-sat)
(exit)
