...
(assert (not (and (or (and (=> (not (= a! b!)) (=> (= empty@ (vstd!set.impl&%0.remove.? T&. T& s! a!)) (=> (= tmp%1 (= (vstd!set.impl&%0.len.? T&. T& empty@) 0)) (and (=> %%location_label%%0 tmp%1) (=> tmp%1 (=> (= tmp%2 (vstd!set.impl&%0.contains.? T&. T& empty@ b!)) (and (=> %%location_label%%1 tmp%2) (=> tmp%2 (=> %%location_label%%2 false))))))))) (=> (not (not (= a! b!))) %%switch_label%%0)) (and (not %%switch_label%%0) (=> %%location_label%%3 (= a! b!)))) (=> (= a! b!) (=> %%location_label%%4 (= a! b!))))))
(check-sat)
(exit)
