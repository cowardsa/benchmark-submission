...
(declare-const tmp%3 alloc!vec.Vec<lib!hashmap_t.CKeyKV./allocator_global%.>.)
...
(assert (not (=> (ens%lib!hashmap_t.impl&%0.to_vec. (%Poly%lib!hashmap_t.CKeyHashMap. self!) v@) (=> (ens%lib!hashmap_t.impl&%0.to_vec. (%Poly%lib!hashmap_t.CKeyHashMap. self!) tmp%1) (=> (ens%lib!marshal_ironsht_specific_v.sorted_keys. tmp%1 a@) (=> (ens%lib!marshal_v.Marshalable._is_marshalable. $ (TYPE%alloc!vec.Vec. $ TYPE%lib!hashmap_t.CKeyKV. $ ALLOCATOR_GLOBAL) (Poly%alloc!vec.Vec<lib!hashmap_t.CKeyKV./allocator_global%.>. v@) tmp%2) (or (and (=> (and (%B tmp%2) a@) (=> (ens%lib!hashmap_t.impl&%0.to_vec. (%Poly%lib!hashmap_t.CKeyHashMap. self!) tmp%3) (and (=> %%location_label%%0 (req%lib!marshal_v.Marshalable.serialized_size. $ (TYPE%alloc!vec.Vec. $ TYPE%lib!hashmap_t.CKeyKV. $ ALLOCATOR_GLOBAL) (Poly%alloc!vec.Vec<lib!hashmap_t.CKeyKV./allocator_global%.>. tmp%3))) (=> (ens%lib!marshal_v.Marshalable.serialized_size. $ (TYPE%alloc!vec.Vec. $ TYPE%lib!hashmap_t.CKeyKV. $ ALLOCATOR_GLOBAL) (Poly%alloc!vec.Vec<lib!hashmap_t.CKeyKV./allocator_global%.>. tmp%3) tmp%4) (=> (ens%lib!marshal_ironsht_specific_v.ckeyhashmap_max_serialized_size_exec. 0 tmp%5) (=> (= tmp%6 (<= (%I tmp%4) tmp%5)) %%switch_label%%0)))))) (=> (not (and (%B tmp%2) a@)) (=> (= tmp%6 false) %%switch_label%%0))) (and (not %%switch_label%%0) (=> (= %return! (B tmp%6)) (=> %%location_label%%1 (= %return! (lib!marshal_v.Marshalable.is_marshalable.? $ TYPE%lib!hashmap_t.CKeyHashMap. self!))))))))))))
(check-sat)
(exit)
