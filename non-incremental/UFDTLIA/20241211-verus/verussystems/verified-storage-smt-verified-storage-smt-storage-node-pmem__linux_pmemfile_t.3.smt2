...
(assert (not (=> (= tmp%2 lib!pmem.linux_pmemfile_t.FileOpenBehavior./CreateNew) (=> (ens%lib!pmem.linux_pmemfile_t.impl&%7.new_internal. path! region_size! tmp%2 persistent_memory_check! tmp%1) (=> (= result! tmp%1) (=> %%location_label%%0 (=> (is-core!result.Result./Ok_ result!) (let ((region$ (%Poly%lib!pmem.linux_pmemfile_t.FileBackedPersistentMemoryRegion. (core!result.Result./Ok/0 (%Poly%core!result.Result. (Poly%core!result.Result. result!)))))) (and (%B (lib!pmem.pmemspec_t.PersistentMemoryRegion.inv.? $ TYPE%lib!pmem.linux_pmemfile_t.FileBackedPersistentMemoryRegion. (Poly%lib!pmem.linux_pmemfile_t.FileBackedPersistentMemoryRegion. region$))) (= (lib!pmem.pmemspec_t.impl&%6.len.? (lib!pmem.pmemspec_t.PersistentMemoryRegion.view.? $ TYPE%lib!pmem.linux_pmemfile_t.FileBackedPersistentMemoryRegion. (Poly%lib!pmem.linux_pmemfile_t.FileBackedPersistentMemoryRegion. region$))) region_size!))))))))))
(check-sat)
(exit)
