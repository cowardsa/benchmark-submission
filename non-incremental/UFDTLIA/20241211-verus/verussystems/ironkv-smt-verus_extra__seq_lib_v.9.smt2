...
(assert (has_type (Poly%fun%2. eq!) (TYPE%fun%2. A&. A& A&. A& $ BOOL)))
(assert (has_type init! B&))
(assert (has_type (Poly%fun%2. f!) (TYPE%fun%2. B&. B& A&. A& B&. B&)))
(assert (= (vstd!seq.Seq.len.? A&. A& s1!) (vstd!seq.Seq.len.? A&. A& s2!)))
(assert (forall ((i$ Poly)) (! (=> (has_type i$ INT) (=> (and (<= 0 (%I i$)) (< (%I i$) (vstd!seq.Seq.len.? A&. A& s1!))) (%B (%%apply%%1 eq! (vstd!seq.Seq.index.? A&. A& s1! i$) (vstd!seq.Seq.index.? A&. A& s2! i$))))) :pattern ((vstd!seq.Seq.index.? A&. A& s1! i$)) :pattern ((vstd!seq.Seq.index.? A&. A& s2! i$)) )))
(assert (forall ((b$ Poly) (a1$ Poly) (a2$ Poly)) (! (=> (and (has_type b$ B&) (has_type a1$ A&) (has_type a2$ A&)) (=> (%B (%%apply%%1 eq! a1$ a2$)) (= (%%apply%%1 f! b$ a1$) (%%apply%%1 f! b$ a2$)))) :pattern ((%%apply%%1 eq! a1$ a2$) (%%apply%%1 f! b$ a1$)) )))
...
(assert (not (=> (= decrease%init0 (vstd!seq.Seq.len.? A&. A& s1!)) (=> (fuel_bool fuel%vstd!seq_lib.impl&%0.fold_left.) (or (and (=> (not (= (vstd!seq.Seq.len.? A&. A& s1!) 0)) (=> (= tmp%1 (vstd!seq_lib.impl&%0.drop_last.? A&. A& s1!)) (=> (= tmp%2 (vstd!seq_lib.impl&%0.drop_last.? A&. A& s2!)) (and (=> %%location_label%%0 (check_decrease_int (let ((s1!$0 tmp%1) (s2!$1 tmp%2) (eq!$2 eq!) (init!$3 init!) (f!$4 f!)) (vstd!seq.Seq.len.? A&. A& s1!$0)) decrease%init0 false)) (and (=> %%location_label%%1 (req%lib!verus_extra.seq_lib_v.lemma_fold_left_on_equiv_seqs. A&. A& B&. B& tmp%1 tmp%2 eq! init! f!)) (=> (ens%lib!verus_extra.seq_lib_v.lemma_fold_left_on_equiv_seqs. A&. A& B&. B& tmp%1 tmp%2 eq! init! f!) %%switch_label%%0)))))) (=> (not (not (= (vstd!seq.Seq.len.? A&. A& s1!) 0))) %%switch_label%%0)) (and (not %%switch_label%%0) (=> %%location_label%%2 (= (vstd!seq_lib.impl&%0.fold_left.? A&. A& B&. B& s1! init! (Poly%fun%2. f!)) (vstd!seq_lib.impl&%0.fold_left.? A&. A& B&. B& s2! init! (Poly%fun%2. f!))))))))))
(check-sat)
(exit)
