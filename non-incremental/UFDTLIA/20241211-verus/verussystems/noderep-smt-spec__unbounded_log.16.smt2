...
(assert (not (and (=> %%location_label%%0 (req%lib!spec.unbounded_log.max_of_set. s!)) (=> (ens%lib!spec.unbounded_log.max_of_set. s! tmp%1) (=> (= r! (nClip (Add tmp%1 1))) (=> %%location_label%%1 (not (vstd!set.impl&%0.contains.? $ NAT (Poly%vstd!set.Set<nat.>. s!) (I r!)))))))))
(check-sat)
(exit)
