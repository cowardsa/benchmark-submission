(set-info :smt-lib-version 2.6)
(set-logic QF_S)
(set-info :source |
Generated by: Hongjian Jiang, Oliver Markgraf, Daniel Stan, Anthony W. Lin, Philipp Ruemmer
Generated on: 2025-04-11
Application: Using String Solvers to solve RMC problems encoded as CHC over Strings
Target solver: cvc5, OSTRICH, Z3, Z3alpha, Z3-noodler
Time limit: 60.0
Benchmarks are extracted by running HornStr https://arg-git.informatik.uni-kl.de/pub/string-chc-lib on all benchmarks
provided in the repository and gathering the string queries sent to the string solvers.
|)
(set-info :license "https://creativecommons.org/licenses/by/4.0/")
(set-info :category "industrial")
(declare-fun varout () String)
(assert (str.in_re varout (re.* (re.union (str.to_re "n") (str.to_re "t")))))
(declare-fun varin () String)
(assert (str.in_re varin (re.* (re.union (str.to_re "n") (str.to_re "t")))))
(declare-fun X () String)
(declare-fun isin () Bool)
(declare-fun isout () Bool)
(assert (let ((a!1 (re.++ (re.++ (re.* (str.to_re "n")) (str.to_re "t"))
                  (re.* (str.to_re "n")))))
  (and (not (str.in_re X a!1)) true (= X varin) isin (not isout))))
(push 1)
(assert (let ((a!1 (re.++ (re.union (str.to_re "n") (str.to_re "t"))
                  (re.* (re.union (str.to_re "n") (str.to_re "t"))))))
(let ((a!2 (re.++ (re.* (re.union (str.to_re "n") (str.to_re "t")))
                  (re.union (str.to_re "") a!1))))
  (str.in_re varin a!2))))
(set-info :status unknown)
(check-sat)
(pop 1)
(push 1)
(assert (let ((a!1 (re.++ (str.to_re "n")
                  (re.++ (re.* (str.to_re "n")) (str.to_re "t")))))
(let ((a!2 (re.++ (str.to_re "t")
                  (re.++ (re.* (str.to_re "n")) (re.union (str.to_re "t") a!1)))))
(let ((a!3 (re.++ (re.union (str.to_re "n") a!2)
                  (re.* (re.union (str.to_re "n") a!2)))))
(let ((a!4 (re.++ (re.* (re.union (str.to_re "n") a!2))
                  (re.union (str.to_re "") a!3))))
(let ((a!5 (re.++ (re.++ (re.* (str.to_re "n")) (re.union (str.to_re "t") a!1))
                  a!4)))
  (str.in_re varin a!5)))))))
(set-info :status unknown)
(check-sat)
(pop 1)
(push 1)
(assert (let ((a!1 (re.union (str.to_re "n")
                     (re.++ (str.to_re "t")
                            (re.union (str.to_re "n") (str.to_re "t"))))))
(let ((a!2 (re.++ (re.* a!1) (re.union (str.to_re "") (re.++ a!1 (re.* a!1))))))
  (str.in_re varin (re.++ (re.union (str.to_re "n") (str.to_re "t")) a!2)))))
(set-info :status unknown)
(check-sat)
(pop 1)
(push 1)
(assert (let ((a!1 (re.++ (str.to_re "n")
                  (re.++ (re.* (str.to_re "n")) (str.to_re "t"))))
      (a!2 (re.union (str.to_re "")
                     (re.++ (str.to_re "n") (re.* (str.to_re "n"))))))
(let ((a!3 (re.++ (re.++ (re.* (str.to_re "n")) (re.union (str.to_re "t") a!1))
                  (re.++ (re.* (str.to_re "n")) a!2)))
      (a!4 (re.++ (re.++ (re.* (str.to_re "n")) (re.union (str.to_re "t") a!1))
                  (re.++ (re.* (str.to_re "n")) (re.union (str.to_re "t") a!1))))
      (a!5 (re.++ (str.to_re "t")
                  (re.++ (re.* (str.to_re "n")) (re.union (str.to_re "t") a!1)))))
(let ((a!6 (re.++ (re.union (str.to_re "n") a!5)
                  (re.++ (re.* (str.to_re "n")) (re.union (str.to_re "t") a!1))))
      (a!7 (re.++ (re.union (str.to_re "n") a!5)
                  (re.++ (re.* (str.to_re "n")) a!2))))
(let ((a!8 (re.union a!7 (re.++ a!6 (re.++ (re.* a!6) a!7)))))
(let ((a!9 (re.union a!3 (re.++ a!4 (re.++ (re.* a!6) a!8)))))
  (str.in_re varin a!9)))))))
(set-info :status unknown)
(check-sat)
(pop 1)
(push 1)
(assert (let ((a!1 (re.* (re.union (str.to_re "t")
                           (re.++ (str.to_re "n") (str.to_re "n"))))))
(let ((a!2 (re.++ (re.union (str.to_re "t")
                            (re.++ (str.to_re "n") (str.to_re "n")))
                  (re.++ a!1 (re.++ (str.to_re "n") (str.to_re "t"))))))
(let ((a!3 (re.++ a!1 (re.union (re.++ (str.to_re "n") (str.to_re "t")) a!2))))
(let ((a!4 (re.union (re.union (str.to_re "n")
                               (re.++ (str.to_re "t") (str.to_re "t")))
                     (re.++ (re.++ (str.to_re "t") (str.to_re "n")) a!3))))
(let ((a!5 (re.++ (re.* a!4) (re.union (str.to_re "") (re.++ a!4 (re.* a!4))))))
(let ((a!6 (re.++ (re.union (str.to_re "t") (re.++ (str.to_re "n") a!3)) a!5)))
  (str.in_re varin a!6))))))))
(set-info :status unknown)
(check-sat)
(pop 1)
(push 1)
(assert (let ((a!1 (re.* (re.union (str.to_re "n")
                           (re.++ (str.to_re "t") (str.to_re "t"))))))
(let ((a!2 (re.++ (re.union (str.to_re "n")
                            (re.++ (str.to_re "t") (str.to_re "t")))
                  a!1)))
(let ((a!3 (re.++ (str.to_re "t") (re.++ a!1 (re.union (str.to_re "") a!2)))))
  (str.in_re varin a!3)))))
(set-info :status unknown)
(check-sat)
(pop 1)
(push 1)
(assert (let ((a!1 (re.union (str.to_re "")
                     (re.++ (str.to_re "n") (re.* (str.to_re "n"))))))
(let ((a!2 (re.++ (str.to_re "t") (re.++ (re.* (str.to_re "n")) a!1))))
  (str.in_re varin a!2))))
(set-info :status unknown)
(check-sat)
(pop 1)
(push 1)
(assert (let ((a!1 (re.++ (str.to_re "n")
                  (re.++ (re.* (str.to_re "n")) (str.to_re "t"))))
      (a!2 (re.union (str.to_re "")
                     (re.++ (str.to_re "n") (re.* (str.to_re "n"))))))
(let ((a!3 (re.++ (re.++ (re.* (str.to_re "n")) (re.union (str.to_re "t") a!1))
                  (re.++ (re.* (str.to_re "n")) a!2))))
  (str.in_re varin a!3))))
(set-info :status unknown)
(check-sat)
(pop 1)
(set-info :status unknown)
(check-sat)
(exit)
